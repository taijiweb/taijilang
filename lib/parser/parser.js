...
exports.Parser = function() {
  var atStatementHead, bigSpc, breakContinueStatement, caseClauseOfSwitchStatement, catchClauseOfTryStatement, char, conjClause, cursor, curve, decimal, elseClause, environment, error, evalParserAttribute, evalParserExpression, evalWhileParsing, expect, expectChar, expectIdentifier, expectIndentConj, expectOneOfWords, expectWord, finallyClause, follow, followMatch, followSequence, indentFromLine, interpolateStringPiece, isIdentifier, itemToParameter, jsIdentifier, keywordTestExpressionBodyStatement, keywordThenElseStatement, leadWordClauseMap, letLikeStatement, lineInfo, lineno, literal, maxLine, maybeOneOfWords, memo, memoIndex, memoMap, newLineAndEmptyLines, newline, newlineFromLine, nonInterpolatedStringLine, number, operatorExpression, paren, parser, predefined, processDataBracetResult, recursive, rollback, rollbackToken, sameIndentFromLine, saveMemo, seperatorList, spaceClauseExpression, spaceComma, spaces, spc, symbol, symbolStopChars, taijiIdentifier, text, thenClause, throwReturnStatement, toParameters, unchangeable, undentFromLine, word;
  parser = this;
  ...
  this.tailComment = function() {
    ...
    if (cursor === lineInfo[lineno].start + indentRow) {
      return;
    }
    start = cursor;
    if (lineno + 1 === maxLine) {
  ...
  this.indentBlockComment = function() {
    var indentRow, line1, start;
    if (cursor !== lineInfo[lineno].start + (indentRow = lineInfo[lineno].indentRow)) {
      return;
    }
    ...
    while (lineno <= maxLine) {
      if (lineInfo[lineno].empty) {
        lineno++;
      } else if (lineInfo[lineno].indentRow > indentRow) {
        lineno++;
      } else {
        break;
      }
  ...
  this.inlineSpaceComment = spc = memo(function() {
  ...
  this.multilineSpaceComment = memo(function() {
    var c, indentRow, line1, multiStart, start;
    start = cursor;
    line1 = lineno;
    indentRow = lineInfo[lineno].indentRow;
    spc();
    ...
    while (c = text[cursor]) {
      if (newLineAndEmptyLines()) {
        continue;
      } else if (indentRow !== lineInfo[lineno].indentRow) {
        break;
  ...
  this.spaceComment = bigSpc = memo(function() {
    return parser.multilineSpaceComment() || spc();
  });
  this.regexp = memo(function() {
    var c, i, indentRow, start;
    if (text[start = cursor] !== '/') {
      return;
    }
    if (cursor === (indentRow = lineInfo[lineno].indentRow)) {
      return;
    }
    c = text[cursor + 1];
    if (c === '*' || c === '/') {
  ...
  this.identifier = memo(function() {
    ...
    } else if (text[cursor] === '\\' && ++cursor && (token = parser.taijiIdentifier())) {
      token.escaped = true;
      return token;
    }
  });
  ...
  this.nonInterpolatedString = memo(function() {
    var indentRow, line1, myIndent, myLineInfo, quote, quoteLength, start;
    ...
    indentRow = null;
    if (cursor === lineInfo[lineno].start + lineInfo[lineno].indentRow) {
      indentRow = lineInfo[lineno].indentRow;
    }
    cursor += quoteLength;
    str = '';
    while (text[cursor]) {
      ...
      if (lineInfo[lineno].empty) {
        ...
        myIndent = myLineInfo.indentRow;
        if (indentRow === null) {
          indentRow = myIndent;
        } else if (myIndent < indentRow) {
  ...
  interpolateStringPiece = function(quote, quoteLength, indentRow, lineIndex) {
    var c, c2, myIndent, x;
    str = '"';
    while (c = text[cursor]) {
      if (text.slice(cursor, cursor + quoteLength) === quote) {
        ...
        if (!lineInfo[lineno].empty && (myIndent = lineInfo[lineno].indentRow) && lineIndex.value++) {
          if (indentRow.value === null) {
            indentRow.value = myIndent;
          } else if (myIndent !== indentRow.value) {
            error('wrong indent in string');
          } else {
            cursor += myIndent;
          }
  ...
  this.interpolatedString = memo(function() {
    var c, indentRow, line1, lineIndex, literalStart, pieces, quote, quoteLength, row, start, x;
    if (text.slice(cursor, cursor + 3) === '"""') {
      quote = '"""';
    } else if (text[cursor] === '"') {
      quote = '"';
    ...
    line1 = lineno;
    indentRow = null;
    if ((row = parser.getRow()) === lineInfo[lineno].indentRow) {
      indentRow = {
        value: row
      };
    } else {
    ...
    while (c = text[cursor]) {
      ...
      if (c === '$') {
        ...
        pieces.push(interpolateStringPiece(quote, quoteLength, indentRow, lineIndex));
      }
    }
    if (!text[cursor]) {
      return error('expect ' + quote + ', unexpected end of input while parsing interpolated string');
  ...
  this.curve = curve = memo(function() {
    var body, indentRow, line1, start;
    start = cursor;
    line1 = lineno;
    indentRow = lineInfo[lineno].indentRow;
    if (text[cursor] !== '{' || text[cursor + 1] === '.') {
  ...
  this.bracket = memo(function() {
    var expList, indentRow, line1, start;
    start = cursor;
    line1 = lineno;
    indentRow = lineInfo[lineno].indentRow;
    if (text[cursor] !== '[') {
  ...
  this.dataBracket = memo(function() {
    var indentRow, line1, result, space, start, x;
    start = cursor;
    line1 = lineno;
    indentRow = lineInfo[lineno].indentRow;
    if (text.slice(cursor, cursor + 2) !== '[\\') {
    ...
    result = [];
    while ((x = parser.dataLine()) && (space = bigSpc())) {
      result.push(x);
      if (lineInfo[lineno].indentRow < indentRow) {
  ...
  this.hashItem = memo(function() {
    var js, key, line1, result, space, space1, space2, start, t, value;
    start = cursor;
    line1 = lineno;
    space1 = bigSpc();
    js = false;
    ...
    if (key = parser.compactClauseExpression()) {
      ...
      if ((space = follow('spaceComment')) && space.indent) {
  ...
  this.hashBlock = memo(function() {
    var c, indentRow, line1, result, row, row1, space, space2, start, x;
    start = cursor;
    line1 = lineno;
    row1 = lineInfo[lineno].indentRow;
    if ((space = bigSpc()) && space.undent) {
      return;
    }
    result = [];
    if (space.indent) {
  ...
  this.atom = function(mode) {
    ...
    if (mode !== 'inStrExp' && (x = parser.string())) {
      x.priority = 1000;
    } else if (x = parser.identifier() || parser.number() || parser.regexp() || parser.delimiterExpression()) {
      x.priority = 1000;
  ...
  this.prefixOperator = function(mode) {
    var line1, op, priInc, space, space2, start, token;
  ...
  this.suffixOperator = function(mode, x, priority) {
    var line1, op, priInc, space, start, token;
    if (!text[cursor]) {
      return;
    }
    ...
    line1 = lineno;
    space = bigSpc();
    if (space.multiline || space.tailComment || space.concatLine) {
      return rollback(start, line1);
    }
    if (space.value) {
      if (priority >= 600) {
  ...
  parser.clauseEnd = function(space) {
    var c, line1, start;
    start = cursor;
    line1 = lineno;
    space = space || bigSpc();
    ...
    if (parser.sentenceEnd(space) || c === ';') {
      rollbackToken(space);
      return true;
    }
  };
  parser.expressionEnd = function(mode, space) {
    var c;
    if (!parser.isClauseExpression(mode)) {
      return;
    }
    return (c = text[cursor]) === ':' && text[cursor + 1] !== ':' || parser.clauseEnd(space) || (mode === 'comClExp' && space.value) || (mode === 'inStrExp' && (c === "'" || c === '"'));
  };
  ...
  this.binaryOperator = function(mode, x, priority, leftAssoc) {
    ...
    start = cursor;
    line1 = lineno;
    if (parser.expressionEnd(mode, space1 = bigSpc())) {
      return rollback(start, line1);
    }
    if (space1.indent || space1.newline) {
      ...
      priInc = 300;
    } else if (cursor === lineInfo[lineno].start + lineInfo[lineno].indentRow) {
      priInc = 0;
    } else {
      priInc = 600;
    }
    ...
    if (parser.expressionEnd(mode, space2 = bigSpc())) {
      return rollback(start, line1);
    }
    if (space2.undent) {
    ...
    if (priInc === 600) {
      ...
      if (space2.value) {
        if (space2.undent) {
          ...
          if ((space3 = bigSpc()) && !space3.undent && text[cursor] && text[cursor] !== ')') {
  ...
  this.binaryCallOperator = function(mode, x, priority, leftAssoc) {
    var line1, space, start;
    start = cursor;
    line1 = lineno;
    if ((space = bigSpc()) && space.value && parser.followParenArguments()) {
      if (mode === 'opExp') {
  ...
  this.binaryIndexOperator = function(mode, x, priority, leftAssoc) {
    ...
    start = cursor;
    line1 = lineno;
    if ((space = bigSpc()) && space.value && follow('bracket')) {
      if (mode === 'opExp') {
  ...
  this.binaryAttributeOperator = function(mode, x, priority, leftAssoc) {
    var line1, space, space2, start;
    start = cursor;
    line1 = lineno;
    if ((space = bigSpc()) && space.value) {
      if (500 <= priority) {
        return rollback(start, line1);
      }
      if (!(text[cursor] === '.' && text[cursor + 1] !== '.')) {
  ...
  this.customBinaryOperators = [this.binaryAttributeOperator, this.binaryCallOperator, this.binaryIndexOperator, this.binaryAtThisAttributeIndexOperator, this.binaryPrototypeAttributeOperator];
  ...
  evalWhileParsing = function(clause) {
    return new Function('__$taiji_$_$parser__', compileExp(['return', clause], environment))(parser);
  };
  evalParserAttribute = function(clause) {
    var code;
    code = compileExp(['return', ['?/', clause]], environment);
    return new Function('__$taiji_$_$parser__', code)(parser);
  };
  evalParserExpression = function(clause) {
    var code;
    code = compileExp(['return', ['?!', clause]], environment);
    return new Function('__$taiji_$_$parser__', code)(parser);
  };
  ...
  this.leadWordClause = function() {
    var clause, fn, key, line1, space, start;
    start = cursor;
    line1 = lineno;
    if (!(key = parser.symbol() || parser.identifier())) {
      return;
    }
    if ((space = spc()) && (!space.value || space.undent)) {
      return rollback(start, line1);
    }
  ...
  this.labelStatement = function() {
    var clause, lbl, line1, space, start;
    start = cursor;
    line1 = lineno;
    ...
    cursor++;
    if ((space = bigSpc()) && (!space.value || space.undent || space.newline)) {
      cursor = start;
      return;
    }
  ...
  this.expectIndentConj = expectIndentConj = function(word, line1, isHeadStatement, options, clauseFn) {
    var indentRow, line2, meetWord, optionalClause, optionalWord, row, space, start2, w;
    start2 = cursor;
    line2 = lineno;
    ...
    if (row === indentRow && lineno !== line1 && !isHeadStatement) {
      if (!optionalClause) {
    ...
    if (row < indentRow) {
      ...
      if (options.indentRow) {
        if (row !== options.indentRow) {
          error('unconsistent indent');
        }
      } else {
        options.indentRow = row;
      }
    }
    w = taijiIdentifier();
    ...
    if (!meetWord) {
      ...
      } else if ((!optionalWord && !optionalClause) || (!optionalClause && optionalWord && space.inline)) {
        error('expect keyword ' + word);
      } else if (!optionalWord) {
        return rollbackToken(space);
  ...
  catchClauseOfTryStatement = function(line1, isHeadStatement, options) {
  ...
  this.identifierLine = function() {
    var result, x;
    result = [];
    while (spc() && !parser.lineEnd() && !follow('newline') && text[cursor] !== ';') {
  ...
  this.identifierList = function() {
    var indentRow, line1, result, row, row0, space, varList;
    line1 = lineno;
    indentRow = lineInfo[line1].indentRow;
    ...
    while (varList = parser.identifierLine()) {
      ...
      space = bigSpc();
      if ((row = lineInfo[lineno].indentRow) <= indentRow) {
        rollbackToken(space);
        break;
      } else if (row !== row0) {
  ...
  this.varInitList = function() {
    var indentRow0, indentRow1, line1, result, row, space, space1, start, x;
    start = cursor;
    line1 = lineno;
    result = [];
    ...
    if (row > indentRow0) {
      indentRow1 = row;
    } else if (space.undent || space.newline) {
  ...
  this.useItem = function() {
    var asName, as_, c, meta, name, x;
    if ((c = text[cursor]) === '#') {
      cursor++;
      meta = 'meta';
      spc();
    }
    if ((x = follow('jsIdentifier')) && x.value === 'from') {
      if (meta) {
  ...
  this.useItemList = seperatorList('useItem', spaceComma);
  ...
  this.keywordToStatementMap = {
    '?': function(isHeadStatement) {
      ...
      line1 = lineno;
      return ['?', parser.lineBlock(), thenClause(line1, isHeadStatement, {})];
    },
    ...
    'include!': function(isHeadStatement) {
      var filePath, parseMethod, with_;
      spc();
      if (with_ = word('with')) {
        spc();
    ...
    'use!': function(isHeadStatement) {
      var alias, as_, from_, items, parseMethod, srcModule, with_;
      spc();
      if (with_ = word('with')) {
        spc();
        parseMethod = expect('taijiIdentifier', 'expect a parser method');
      }
      items = parser.useItemList();
      spc();
      if (items.length) {
      ...
      spc();
      srcModule = parser.compactClauseExpression();
      spc();
      if (as_ = literal('as')) {
        spc();
        alias = expectIdentifier('expect an alias after "as" for module');
      }
      return ['use!'].concat([srcModule, alias, parseMethod, items]);
    },
    ...
    'for': function(isHeadStatement) {
      var inOf, init, kw, line1, list, name1, name2, step, test, token, value;
      line1 = lineno;
      spc();
      if (text[cursor] === '(' && cursor++) {
        init = parser.clause();
      ...
      return [kw, name1, name2, list, thenClause(line1, isHeadStatement, {})];
    },
    'do': function(isHeadStatement) {
      var body, conj, indentRow, line1, tailClause;
      line1 = lineno;
      spc();
      indentRow = lineInfo[lineno].indentRow;
      body = parser.lineBlock();
      if (newlineFromLine(line1) && !isHeadStatement) {
        return body;
      }
    ...
    'try': function(isHeadStatement) {
      var catchClauses, catch_, else_, final, line1, options, test;
      line1 = lineno;
      ...
      options = {};
      catchClauses = ['list!'];
      while (catch_ = catchClauseOfTryStatement(line1, isHeadStatement, options)) {
        catchClauses.push(catch_);
      }
      else_ = elseClause(line1, isHeadStatement, options);
      final = finallyClause(line1, isHeadStatement, options);
      return ['try', begin(test), catchClauses, else_, final];
    },
    'class': function(isHeadStatement) {
      ...
      if (parser.conjunction('extends')) {
        ...
        if (supers.type === PAREN) {
          supers = getOperatorExpression(supers);
        } else {
          supers = [getOperatorExpression(supers)];
        }
      } else {
      ...
      body = parser.lineBlock();
      return ['class', supers].concat(body);
    }
  };
  ...
  this.makeAssignClause = function(assignLeftSide, assignSymbol, assignRightSide) {
    return function() {
      var left, line1, right, space, start, token;
      start = cursor;
      line1 = lineno;
  ...
  this.indentClause = memo(function() {
    var blk, head, line1, space, start;
    start = cursor;
    line1 = lineno;
    if (!(head = parser.sequenceClause())) {
      return;
  ...
  this.unaryExpressionClause = memo(function() {
    ...
    start = cursor;
    line1 = lineno;
    if ((head = parser.compactClauseExpression()) && spc() && (x = parser.spaceClauseExpression()) && parser.clauseEnd()) {
      if (text[cursor] === ',') {
        cursor++;
      }
  ...
  this.defaultSymbolOfDefinition = function() {
    var x, xTail;
    if (x = parser.symbol()) {
      if ((xTail = x.value.slice(x.value.length - 2)) === '->' || xTail === '=>') {
        return x;
      } else {
  ...
  this.customClauseList = ['statement', 'labelStatement', 'leadWordClause', 'assignClause', 'colonClause', 'indentClause', 'expressionClause', 'unaryExpressionClause'];
  ...
  this.sentenceEnd = function(space) {
    space = space || bigSpc();
    if (parser.lineEnd()) {
      return true;
    }
  ...
  this.lineCommentBlock = memo(function() {
    ...
    if (comment = parser.lineComment()) {
      if (comment.indent) {
        ...
        return [
          extend(['lineComment!', '"' + text.slice(start, cursor) + '"'], {
            start: start,
            stop: cursor,
            line: lineno
          })
  ...
  this.blockWithoutIndentHead = function() {
    var indentRow, result, space, x, x0;
    indentRow = lineInfo[lineno].indentRow;
    result = [];
    while ((x = parser.line()) && (space = bigSpc())) {
  ...
  this.lineBlock = function() {
    var cursor2, indentRow, line, line1, line2, row, space1, start;
    start = cursor;
    line1 = lineno;
    indentRow = lineInfo[lineno].indentRow;
    space1 = bigSpc();
  ...
  this.dataClause = function() {
    var clause, item, space, sym;
    if (parser.clauseEnd(space = bigSpc())) {
      return;
    }
    clause = [];
    while (1) {
  ...
  this.moduleHeader = function() {
    ...
    lineno = 2;
    while (lineno <= maxLine && (lineInfo[lineno].indentRow > 0 || lineInfo[lineno].empty)) {
      lineno++;
    }
    if (lineno > maxLine) {
      cursor = text.length;
  ...
  this.indentFromLine = indentFromLine = function(line1) {
    return lineInfo[lineno].indentRow > lineInfo[line1].indentRow;
  };
  ...
  this.newlineFromLine = newlineFromLine = function(line1) {
    return lineno !== line1 && lineInfo[lineno].indentRow === lineInfo[line1].indentRow;
  };
  this.getRow = function() {
    return cursor - lineInfo[lineno].start;
  };
  this.preparse = function() {
    var atLineHead, c, diffSpace, i, indentLine, indentRow, indentStack, indentStackIndex, line, lineHeadChar, prevIndentLine, row;
    i = 0;
    line = 1;
    row = 0;
  ...
  this.init = function(data, cur, env) {
    ...
    return this.meetEllipsis = false;
  };
