      ...
      if (char === '"') {
        if (text[cursor + 1] === '"') {
          if (text[cursor + 2] === '"') {
            ...
            pieces.atom = true;
            pieces.cursor = cur;
            pieces.stopCursor = cursor;
            pieces.line = line;
            pieces.stopLine = line;
            return pieces;
          } else {
            cursor += 2;
            char = text[cursor];
  ...
  leftInterpolateString = function() {
    ...
    while (char) {
      if (char === '"') {
        ...
        pieces.type = tokenType = INTERPOLATE_STRING;
        pieces.kind = LIST;
        pieces.transformed = true;
        pieces.atom = true;
        pieces.cursor = cur;
        pieces.stopCursor = cursor;
        pieces.line = line;
        pieces.stopLine = lineno;
        return pieces;
      } else if (char === '\n') {
        if (!concatenating) {
          s += char;
