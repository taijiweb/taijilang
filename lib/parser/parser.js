...
_ref2 = require('./operator'), prefixOperatorDict = _ref2.prefixOperatorDict, suffixOperatorDict = _ref2.suffixOperatorDict, binaryOperatorDict = _ref2.binaryOperatorDict;
...
exports.Parser = function() {
  ...
  this.suffixOperator = function(mode, x) {
    ...
    if ((op = suffixOperatorDict[token.value])) {
      opToken = token;
      nextToken();
      if (tokenType === SPACE || tokenType === NEWLINE || tokenType === INDENT || tokenType === UNDENT || tokenType === EOI || tokenType === RIGHT_DELIMITER || (value = token.value) === ':' || value === ',' || value === ';') {
        return {
          value: op.value || opToken.value,
          start: opToken,
          stop: token,
  ...
  this.binaryOperator = function(mode, x) {
    var cur, m, memoTag, op, opToken, opValue, pri, priInc, result, rightAssoc, start, tkn, type1, value;
    ...
    switch (tokenType) {
      ...
      case PUNCTUATION:
        ...
        if (priInc === 0) {
          if (opValue === ',' || opValue === ':') {
            syntaxError('binary operator ' + (op.value || opValue) + ' should not be at begin of line');
          }
        }
        break;
      default:
  ...
  indentExpression = function() {
    ...
    indentExp = ['indentExpression!', indentExp];
    indentExp.priority = 1000;
    indentExp.start = start;
    indentExp.atom = true;
  ...
  this.keyword2statement = keyword2statement = {
    ...
    'class': function(isHeadStatement) {
      var body, name, result, superClass;
      nextToken();
      if (tokenType === SPACE) {
        nextToken();
      }
  ...
  this.clause = function() {
    var blk, clause, clause0, clauses, definition, exp, head, isStatementHead, op, params, result, right, start, tkn, tokenAfterOperator, type, value;
    if (tokenType === SPACE) {
      nextToken();
      type = tokenType;
    }
    ...
    if ((value = token.value) === ',') {
      ...
      if (clauses.length === 1) {
        clause0 = clauses[0];
        if (clause0 instanceof Array && clause0.type !== BRACKET) {
          clauses = clause0;
        }
