...
exports.Parser = function() {
  ...
  tokenFnMap[','] = tokenFnMap[';'] = tokenOnPunctuationChar = function() {
    return {
      value: char,
      type: PUNCTUATION,
      lineno: lexLineno,
  ...
  tokenFnMap["'"] = tokenOnSingleQuoteChar = function() {
    ...
    if (char === "'") {
      ...
      return chainToken(leftNonInterpolatedString());
    }
  };
  ...
  tokenFnMap['"'] = tokenOnDoubleQuoteChar = function() {
    ...
    if (char === '"') {
      ...
      return chainToken(leftInterpolatedString());
    }
  };
  ...
  tokenFnMap['['] = tokenOnLeftBracketChar = function() {
    var bracketVariantFn, col, cur, expList, ind, line, start, type;
    cur = cursor;
    cursor++;
    line = lineno;
    col = column;
  ...
  this.delimiterExpression = function() {
  ...
  parser.expressionEnd = function(mode, spac) {
    ...
    return (c = text[cursor]) === ':' && text[cursor + 1] !== ':' || parser.clauseEnd(spac) || (mode === 'comClExp' && spac.text) || (mode === 'inStrExp' && (c === "'" || c === '"'));
  };
  ...
  binaryDictOperator = function(mode, x) {
    ...
    if (txt2 === '.') {
      if (priInc === 600) {
        error('unexpected "." after binary operator ' + opToken.text + ', here should be spaces, comment or newline');
      } else {
        matchToken();
      }
    }
    ...
    if ((type = token.type) === UNDENT) {
      error('unexpected undent after binary operator "' + opToken.text + '"');
    } else if (type === EOI) {
    ...
    if (priInc === 600) {
      if (type === SPACE) {
        if (op.text === ',') {
          ...
          error('unexpected spaces or new lines after binary operator "' + op.text + '" before which there is no space.');
        }
      }
      ...
      if (type === UNDENT) {
        error('unexpceted undent after binary operator ' + op.text);
      } else if (type === NEWLINE) {
        priInc = 0;
      } else if (type === INDENT) {
        priInc = 0;
        ...
        if (mode === OPERATOR_EXPRESSION) {
          error('binary operator ' + op.text + ' should have spaces at its right side.');
        } else {
          token = start;
        }
      }
  ...
  indentExpression = function() {
    ...
    tkn = matchToken();
    if ((type = tkn.type) !== UNDENT && type !== EOI && type.text !== ')') {
  ...
  customBinaryOperatorFnMap[IDENTIFIER] = function(mode, x) {
    var cur;
    cur = token.cursor;
    if ((x.value === '@' && x.stopCursor === cur - 1) || (text.slice(cur - 2, cur) === '::' && text[cur - 3] !== ':')) {
      return {
        symbol: 'attribute!',
        type: SYMBOL,
        priority: 800,
  ...
  this.expression = expression = function(mode, priority, leftAssoc) {
    var indexMode, op, opPri, result, start, x, y;
    ...
    while ((op = parser.suffixOperator(mode, x)) && op.priority >= priority) {
      x = wrapResult(makeOperatorExpression('suffix!', op, x), {
        start: start,
        stop: token
      });
    }
    while ((op = parser.binaryOperator(mode, x)) && ((leftAssoc && (opPri = op.priority) > priority) || (!leftAssoc && opPri >= priority))) {
  ...
  this.leadWordClause = function() {
    ...
    if ((spac = space()) && (!spac.text || spac.undent)) {
      return rollback(start, line1);
    }
    if (!(fn = leadWordClauseMap[key.text])) {
      return rollback(start, line1);
    }
  ...
  this.defaultSymbolOfDefinition = function() {
    var x, xTail, xValue;
    if ((x = parser.symbol())) {
      if ((xValue = x.text) && xValue[0] !== '\\' && ((xTail = xValue.slice(xValue.length - 2)) === '->' || xTail === '=>')) {
        return x;
      } else {
  ...
  this.clause = function() {
    var clause, line1, matcher, start, x, _k, _len2, _ref5;
    start = cursor;
    line1 = lineno;
    if (parser.clauseEnd()) {
      return;
    }
    _ref5 = parser.customClauseList;
    for (_k = 0, _len2 = _ref5.length; _k < _len2; _k++) {
      matcher = _ref5[_k];
      if (x = parser[matcher]()) {
        return x;
