...
exports.Parser = function() {
  var atLineHead, atStatementHead, binaryOperatorMemoIndex, bracketVariantMap, breakContinueStatement, c, caseClauseOfSwitchStatement, catchClause, char, concatenateLine, concatenating, conjClause, cursor, curveVariantMap, decimal, definitionSymbolBody, elseClause, endCursorOfDynamicBlockStack, environment, eoi, error, expectChar, expectIdentifier, expectOneOfWords, expectWord, expression, expressionMemoIndex, finallyClause, firstLevelConjunctionThen, fn, hashBlock, hashItem, hashLine, hashLineBlock, indent, indentExpression, interolateStringNumber, itemToParameter, key, keyword2statement, keywordTestExpressionBodyStatement, keywordThenElseStatement, leadTokenClause, leadWordClauseMap, leftCBlockComment, leftIndentBlockComment, leftInterpolateString, leftNonInterpolatedString, leftRawInterpolateString, leftRawNonInterpolatedString, leftRegexp, letLikeStatement, lexError, lexIndent, lineStart, lineno, literal, matchChar, matchToken, maybeOneOfWords, memoIndex, memoMap, newLineAndEmptyLines, newline, nextToken, nonInterpolatedStringLine, nullToken, operatorExpression, parenVariantMap, parser, predefined, rawNonInterpolatedStringLine, seperatorList, skipInlineSpace, skipSpace, skipSpaceLines, skipToken, spaceClauseExpression, spaceComma, spaces, symbol2clause, symbolStopChars, text, thenClause, throwReturnStatement, toParameters, token, tokenFnMap, tokenOnAtChar, tokenOnBackSlashChar, tokenOnColonChar, tokenOnCommaChar, tokenOnDoubleQuoteChar, tokenOnForwardSlashChar, tokenOnIdentifierChar, tokenOnLeftBracketChar, tokenOnLeftParenChar, tokenOnNewlineChar, tokenOnNumberChar, tokenOnRightDelimiterChar, tokenOnSemiColonChar, tokenOnSingleQuoteChar, tokenOnSpaceChar, tokenOnSymbolChar, unchangeable, varInitLine, word, _i, _j, _len, _len1, _ref3, _ref4;
  parser = this;
  ...
  bracketVariantMap = {};
  tokenFnMap['{'] = function() {
    var body, column, cur, curveVariantFn, line, start, tkn;
    cur = cursor;
    char = text[++cursor];
    line = lineno;
    column = cursor - lineStart;
  ...
  firstLevelConjunctionThen = function() {
    ...
    if (token.type === NEWLINE) {
      return 'unexpected new line, expect "then" instead';
    } else if (token.type === INDENT) {
      return 'unexpected indent, expect "then" instead';
    } else if (token.type === UNDENT) {
      return error('unexpected undent, expect "then" instead');
    } else if (token.type === EOI) {
      return error('unexpected end of input, expect "then" instead');
    } else if (token.type === CONJUNCTION) {
      if (token.value === "then") {
        nextToken();
        return true;
      } else {
        return error('unexpected conjunction "' + token.value + '", expect "then" instead');
      }
    } else {
      return error('expect "then"');
    }
  };
      ...
      while (1) {
        ...
        if (token.type === EOI) {
          ...
          if (indent === ind1) {
            ...
            error('unconsistent indent in var initialization block');
          }
        } else {
          break;
        }
  ...
  this.keyword2statement = keyword2statement = {
    '%': function(isHeadStatement) {
      ...
      if (token.value === 'then' && nextToken()) {
        ...
        if (indent < ind) {
          error('expect same or less indent after custom parsing');
        }
        return result;
      } else {
        return error('expect the conjunction "then"');
      }
    },
    ...
    'import!': function(isHeadStatement) {
      var alias, alias2, from_, item, items, metaAlias, metaImportList, parseMethod, runtimeImportList, srcModule, sym2, symValue, x, _k, _l, _len2, _len3;
      if (token.type === SPACE) {
        nextToken();
      }
    ...
    'for': function(isHeadStatement) {
      ...
      if (token.type !== IDENTIFIER) {
        error('expect "in", "of" or index variable name');
      }
  ...
  this.clause = function() {
    ...
    } else if ((tkn = token) && (exp = parser.spaceClauseExpression())) {
      if (exp.priority <= 600 && (type = head.type) !== NUMBER && type !== NON_INTERPOLATE_STRING && type !== INTERPOLATE_STRING && type !== BRACKET && type !== HASH) {
        if ((value = token.value) === ',') {
          nextToken();
          if (token.type === SPACE) {
            nextToken();
    ...
    if (op && op.assign) {
      ...
      if (type === UNDENT) {
        error('unexpected undent after assign symbol' + op.value);
      } else if (type === NEWLINE) {
        error('unexpected new line after assign symbol' + op.value);
      } else if (type === EOI) {
        error('unexpected end of input' + op.value);
      }
    ...
    if ((value = token.value) === ',') {
      ...
      if (clauses.length === 1) {
        ...
        error('expected arguments list after ":"');
      }
