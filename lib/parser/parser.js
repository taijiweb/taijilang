...
exports.Parser = function() {
  ...
  tokenFnMap['('] = tokenOnLeftParenChar = function() {
    ...
    if ((parenVariantFn = parenVariantMap[token.value])) {
      ...
      if (token.type === SPACE || token.type === NEWLINE || token.type === INDENT) {
        nextToken();
      }
      if (token.value === ')') {
        token = {
  ...
  hashLine = function() {
    ...
    while ((x = parser.hashItem())) {
      ...
      if ((value = token.value) === ';') {
        ...
        nextToken();
        break;
      } else if (token.type === UNDENT || value === '}') {
        break;
  ...
  this.hashItem = hashItem = function() {
    ...
    if (key = parser.hashKeyExpression()) {
      if (token.type === NEWLINE || token.type === UNDENT) {
  ...
  this.prefixOperator = function(mode) {
    ...
    if (token.type === INDENT) {
      ...
      if (token.value === '.') {
        nextToken();
        if (token.type === SPACE || token.type === NEWLINE || token.type === UNDENT || token.type === INDENT || token.type === EOI) {
  ...
  this.suffixOperator = function(mode, x) {
    ...
    if ((op = suffixOperatorDict[token.value])) {
      ...
      opToken = token;
      nextToken();
      if (token.type === SPACE || token.type === NEWLINE || token.type === INDENT || token.type === UNDENT || token.type === EOI || token.type === RIGHT_DELIMITER || (value = token.value) === ':' || value === ',' || value === ';') {
        return extendSyntaxInfo(opToken, opToken);
      } else {
        token = opToken;
      }
  ...
  this.binaryOperator = function(mode, x) {
    var cur, m, op, opToken, opValue, pri, priInc, start, tkn, type1, value;
    ...
    switch (type1 = token.type) {
      ...
      case SYMBOL:
        ...
        if ((value = tkn.value) === '#' && nextToken()) {
          ...
          } else if (token.type === SPACE || token.type === NEWLINE || token.type === INDENT || token.type === UNDENT || token.type === EOI) {
            if (mode === OPERATOR_EXPRESSION) {
          ...
          } else if (token.type === NUMBER || token.type === NON_INTERPOLATE_STRING || token.type === INTERPOLATE_STRING) {
            return {
              symbol: 'index!',
              type: SYMBOL,
              start: tkn,
  ...
  indentExpression = function() {
    ...
    if (token.type !== UNDENT && token.type !== EOI && token.value !== ')') {
  ...
  this.keyword2statement = keyword2statement = {
    ...
    'var': function(isHeadStatement) {
      ...
      if (token.type !== NEWLINE && token.type !== UNDENT && token.type !== EOI && token.type !== CONJUNCTION && token.type !== RIGHT_DELIMITER && token.type !== PUNCTUATION) {
    ...
    if (op = parser.binaryOperator(SPACE_CLAUSE_EXPRESSION)) {
      ...
      if ((exp = parser.spaceClauseExpression())) {
        if ((value = token.value) === ',') {
          ...
          return exp;
        } else if (value !== ';' && token.type !== NEWLINE && token.type !== UNDENT && token.type !== EOI) {
          if (exp.priority < 600) {
      ...
      if ((exp = parser.spaceClauseExpression()) && exp.priority <= 600) {
        ...
        } else if (value !== ';' && token.type !== NEWLINE && type !== UNDENT && type !== EOI) {
  ...
  this.sentence = function() {
    var result, start;
    start = token;
    if (token.type === EOI || token.type === INDENT || token.type === UNDENT || token.type === NEWLINE || token.type === RIGHT_DELIMITER || token.type === CONJUNCTION) {
      return;
    }
  ...
  this.line = function() {
    ...
    if (token.type === UNDENT || token.type === RIGHT_DELIMITER || token.type === CONJUNCTION || token.type === EOI) {
      return;
    }
    if (token.type === INDENT) {
      return parser.block(indent);
    }
    if (token.type === CODE_BLOCK_COMMENT_LEAD_SYMBOL) {
      start = token;
      nextToken();
      if (token.type === SPACE) {
        nextToken();
