...
exports.Parser = function() {
  var atLineHead, atStatementHead, binaryOperatorMemoIndex, bracketVariantMap, breakContinueStatement, c, char, concatenateLine, concatenating, cursor, curveVariantMap, decimal, definitionSymbolBody, environment, eoi, expectThen, expression, expressionMemoIndex, fn, hashBlock, hashItem, hashLine, hashLineBlock, indent, indentExpression, itemToParameter, key, keyword2statement, keywordThenElseStatement, leadTokenClause, leadWordClauseMap, leftCBlockComment, leftIndentBlockComment, leftInterpolateString, leftNonInterpolatedString, leftRawInterpolateString, leftRawNonInterpolatedString, leftRegexp, letLikeStatement, lexError, lexIndent, lineStart, lineno, literal, matchChar, matchToken, maybeConjunction, maybeIndentConjunction, memoIndex, memoMap, newLineAndEmptyLines, newline, nextToken, nonInterpolatedStringLine, nullToken, operatorExpression, parenVariantMap, parser, rawNonInterpolatedStringLine, seperatorList, skipInlineSpace, skipSpace, skipSpaceLines, skipToken, spaceClauseExpression, spaceComma, spaces, symbol2clause, symbolStopChars, syntaxError, text, throwReturnStatement, token, tokenFnMap, tokenOnAtChar, tokenOnBackSlashChar, tokenOnColonChar, tokenOnCommaChar, tokenOnDoubleQuoteChar, tokenOnForwardSlashChar, tokenOnIdentifierChar, tokenOnLeftBracketChar, tokenOnLeftParenChar, tokenOnNewlineChar, tokenOnNumberChar, tokenOnRightDelimiterChar, tokenOnSemiColonChar, tokenOnSharpChar, tokenOnSingleQuoteChar, tokenOnSpaceChar, tokenOnSymbolChar, tokenType, varInitLine, whileTestStatement, _i, _j, _k, _l, _len, _len1, _len2, _len3, _ref2, _ref3, _ref4, _ref5;
  parser = this;
  text = '';
  cursor = 0;
  char = '';
  ...
  tokenFnMap['['] = tokenOnLeftBracketChar = function() {
    var bracketVariantFn, column, cur, exp, line, prev;
    cur = cursor;
    char = text[++cursor];
    line = lineno;
    column = cursor - lineStart;
  ...
  this.expression = expression = function(mode, priority, leftAssoc) {
    var m, memoTag, op, opPri, result, start, tkn, tkn1, tkn2, x, y;
      ...
      if ((op = parser.binaryOperator(mode, x))) {
        if ((opPri = op.priority) > priority || (opPri === priority && !leftAssoc)) {
          ...
          if (y) {
            ...
            x.expressionType = BINARY;
            x.priority = op.priority;
            x.rightAssoc = op.rightAssoc;
            x.start = op.start;
            x.stop = op.stop || op.start;
            x.kind = LIST;
          } else {
            token = tkn2;
            tokenType = token.type;
  ...
  letLikeStatement = function(keyword) {
    return function(isHeadStatement) {
      var body, ind, result, start, varDesc;
      start = token;
      nextToken();
      ind = indent;
      if (tokenType === SPACE) {
  ...
  this.keyword2statement = keyword2statement = {
    ...
    'for': function(isHeadStatement) {
      ...
      if (char === '(' && (char = text[++cursor])) {
        ...
        body = parser.block() || parser.line();
        return {
          value: ['cFor!', init, test, step, begin(body)],
          start: start,
          stop: token
        };
      }
    ...
    'do': function(isHeadStatement) {
      ...
      if (conjValue === 'where') {
        return {
          value: ['let', tailClause, begin(body)],
          start: start,
          stop: token
        };
      } else if (conjValue === 'when') {
        return {
          value: ['doWhile!', begin(body), tailClause],
          start: start,
          stop: token
        };
      } else {
        return {
          value: ['doWhile!', begin(body), ['prefix', '!', tailClause]],
          start: start,
          stop: token
        };
      }
    ...
    'switch': function(isHeadStatement) {
      var body, caseValues, cases, else_, exp, ind, indentInfo, result, start, test;
      start = token;
      ind = indent;
      nextToken();
      if (tokenType === SPACE) {
    ...
    'try': function(isHeadStatement) {
      ...
      if (maybeConjunction("finally", isHeadStatement, ind)) {
        ...
        result = ['try', begin(test), catchVar, begin(catch_), begin(final)];
      }
      result.start = start;
      result.stop = token;
      return result;
    ...
    'class': function(isHeadStatement) {
      var body, name, result, start, superClass;
      start = token;
      nextToken();
      if (tokenType === SPACE) {
        nextToken();
  ...
  this.clause = function() {
    var blk, clause, clauses, definition, exp, head, isStatementHead, op, opToken, params, result, start, tkn, tokenAfterOperator, type, value;
    if (tokenType === SPACE) {
      nextToken();
      type = tokenType;
    }
    ...
    if ((op = binaryOperatorDict[token.value]) && op.assign) {
      ...
      result = [opToken, head, parser.block() || parser.clause()];
      result.start = start;
      result.stop = token;
      return result;
    } else if (token.value === '#' && nextToken()) {
    ...
    if ((op = binaryOperatorDict[token.value]) && op.definition) {
      ...
      } else if (clause instanceof Array && clause.length > 1) {
  ...
  this.module = function() {
    ...
    scriptDirective = [
      {
        value: 'scriptDirective!',
        kind: SYMBOL
      }, parser.binShellDirective()
    ];
