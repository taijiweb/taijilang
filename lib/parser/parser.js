...
exports.Parser = function() {
  ...
  this.importItemList = function() {
    var items, lst, result, _i, _len;
    lst = importItemFn();
    result = [];
  ...
  this.keywordToStatementMap = {
    ...
    'import!': function(isHeadStatement) {
      var alias, alias2, as_, from_, items, metaAlias, parseMethod, srcModule, sym, sym2, symValue, with_;
      spc();
      ...
      if (as_ = literal('as')) {
        ...
        sym = parser.symbol();
        if (sym) {
          if ((symValue = sym.value) !== '#' && sym.valu !== '#/') {
      ...
      return ['import!'].concat([srcModule, parseMethod, alias, metaAlias, items]);
    },
