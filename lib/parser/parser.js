...
exports.Parser = function() {
  var atLineHead, atStatementHead, binaryOperatorMemoIndex, bracketVariantMap, breakContinueStatement, c, caseClauseOfSwitchStatement, catchClause, char, concatenateLine, concatenating, conjClause, cursor, curveVariantMap, decimal, definitionSymbolBody, elseClause, endCursorOfDynamicBlockStack, environment, eoi, expression, expressionMemoIndex, finallyClause, firstLevelConjunctionThen, fn, hashBlock, hashItem, hashLine, hashLineBlock, indent, indentExpression, interolateStringNumber, itemToParameter, key, keyword2statement, keywordTestExpressionBodyStatement, keywordThenElseStatement, leadTokenClause, leadWordClauseMap, leftCBlockComment, leftIndentBlockComment, leftInterpolateString, leftNonInterpolatedString, leftRawInterpolateString, leftRawNonInterpolatedString, leftRegexp, letLikeStatement, lexError, lexIndent, lineStart, lineno, literal, matchChar, matchToken, memoIndex, memoMap, newLineAndEmptyLines, newline, nextToken, nonInterpolatedStringLine, nullToken, operatorExpression, optionalFirstLevelConjunction, parenVariantMap, parser, predefined, rawNonInterpolatedStringLine, seperatorList, skipInlineSpace, skipSpace, skipSpaceLines, skipToken, spaceClauseExpression, spaceComma, spaces, symbol2clause, symbolStopChars, syntaxError, text, thenClause, throwReturnStatement, toParameters, token, tokenFnMap, tokenOnAtChar, tokenOnBackSlashChar, tokenOnColonChar, tokenOnCommaChar, tokenOnDoubleQuoteChar, tokenOnForwardSlashChar, tokenOnIdentifierChar, tokenOnLeftBracketChar, tokenOnLeftParenChar, tokenOnNewlineChar, tokenOnNumberChar, tokenOnRightDelimiterChar, tokenOnSemiColonChar, tokenOnSingleQuoteChar, tokenOnSpaceChar, tokenOnSymbolChar, unchangeable, varInitLine, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _m, _ref3, _ref4, _ref5, _ref6;
  parser = this;
  ...
  for (_j = 0, _len1 = firstIdentifierCharSet.length; _j < _len1; _j++) {
    c = firstIdentifierCharSet[_j];
    symbolStopChars[c] = true;
  }
  _ref4 = '0123456789';
  ...
  tokenFnMap['\\'] = tokenOnBackSlashChar = function() {
    var column, cur, line, tkn, _l, _len3, _ref5;
    cur = cursor;
    column = cursor - lineStart;
    char = text[++cursor];
    if (char === '\n' || char === '\r') {
  ...
  tokenFnMap['('] = tokenOnLeftParenChar = function() {
    var column, cur, exp, ind, line, parenVariantFn, start, tkn, type;
    cur = cursor;
    line = lineno;
    column = cursor - lineStart;
    char = text[++cursor];
  ...
  tokenFnMap['['] = tokenOnLeftBracketChar = function() {
    var bracketVariantFn, column, cur, expList, line, start, tkn;
    cur = cursor;
    char = text[++cursor];
    line = lineno;
    column = cursor - lineStart;
  ...
  bracketVariantMap = {};
  tokenFnMap['{'] = function() {
    var body, column, cur, curveVariantFn, ind, line, start, tkn;
    cur = cursor;
    char = text[++cursor];
    line = lineno;
    column = cursor - lineStart;
    ...
    while (1) {
      ...
      if ((char = text[cursor]) === '.') {
        ...
        if ((tkn = tokenOnLeftBracketChar()) && token.type === BRACKET) {
          ...
          syntaxError('error while parsing "[" leading interpolate expression in double qoute string');
        }
      } else {
        cursor = cur;
        break;
  ...
  firstLevelConjunctionThen = function() {
    var ind;
    ind = indent;
    if (token.type === SPACE) {
      nextToken();
    ...
    if (token.type === NEWLINE) {
      return 'unexpected new line, expect "then"';
    } else if (token.type === INDENT) {
      return 'unexpected indent, expect "then"';
    } else if (token.type === UNDENT) {
      return syntaxError('unexpected undent, expect "then"');
    } else if (token.type === EOI) {
      return syntaxError('unexpected end of input, expect "then"');
  ...
  optionalFirstLevelConjunction = function(conj, isHeadStatement, ind) {
    ...
    if (indent === ind && token.type === CONJUNCTION && token.value === conj) {
      conj = token;
      nextToken();
      return conj;
    }
  ...
  keywordThenElseStatement = function(keyword) {
    return function(isHeadStatement) {
      var else_, ind, start, test, then_;
      start = token;
      ind = indent;
      nextToken();
      if (token.type === SPACE) {
  ...
  this.importItem = function() {
    ...
    if ((as_ = taijiIdentifier())) {
      if (as_.text === 'from') {
        ...
        syntaxError('unexpected word ' + as_.text + ', expect "as", "," or "from [module path...]"');
      } else {
        if (token.type === SPACE) {
          nextToken();
        }
        ...
        if (symValue === '#/' && !symValue2) {
          ...
          sym3 = parser.symbol();
          if (!sym3) {
            syntaxError('expect # after "#/' + name.text + ' as ' + asName.text + '"');
          } else if (sym3.text !== '#') {
            syntaxError('unexpected ' + sym3.text + ' after "#/' + name.text + 'as ' + asName.text + '"');
          }
  ...
  this.keyword2statement = keyword2statement = {
    ...
    'import!': function(isHeadStatement) {
      var alias, alias2, from_, item, items, metaAlias, metaImportList, parseMethod, runtimeImportList, srcModule, sym2, symValue, x, _len5, _len6, _n, _o;
      if (token.type === SPACE) {
        nextToken();
      }
      ...
      if (token.value === 'as') {
        ...
        if (token.type === SYMBOL && (symValue = token.value) && symValue !== '#' && symbolValue !== '#/') {
        ...
        sym2 = parser.symbol();
        if (sym && sym2) {
          syntaxError('unexpected symbol after meta alias');
        }
        if (token.type === SPACE) {
          nextToken();
        }
      ...
      for (_n = 0, _len5 = items.length; _n < _len5; _n++) {
        item = items[_n];
        for (_o = 0, _len6 = item.length; _o < _len6; _o++) {
          x = item[_o];
          if (x[2]) {
    ...
    'for': function(isHeadStatement) {
      var body, clause, inOf, init, kw, name1, name2, obj, start, step, test, value;
      start = token;
      skipToken();
      skipSpace();
    ...
    'do': function(isHeadStatement) {
      ...
      if (indent === ind) {
        ...
        if (!isHeadStatement && token.type === CONJUNCTION && (conjValue = token.value) === 'where' || conjValue === 'when' || conjValue === 'until') {
          conj = token;
          nextToken();
        }
      } else if (indent > ind) {
    ...
    'try': function(isHeadStatement) {
      ...
      if (!catch_) {
        syntaxError('expect a catch clause for try-catch statement');
      }
  ...
  this.clause = function() {
    var blk, clause, clause0, clauseValue, clauses, definition, exp, head, op, params, result, right, start, tkn, tokenAfterOperator, type, value;
    ...
    switch (token.type) {
      case KEYWORD:
        return keyword2statement[token.value](atStatementHead);
  ...
  this.sentence = function() {
    ...
    start = token;
    if ((type = token.type) === EOI || type === UNDENT || type === NEWLINE || type === RIGHT_DELIMITER || type === CONJUNCTION) {
      return;
    }
    ...
    while ((x = parser.line())) {
      ...
      if (token.type === UNDENT) {
        if (indent === dent) {
          ...
          syntaxError('wrong indent column');
        } else {
          continue;
        }
      } else {
