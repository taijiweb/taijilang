var BINARY, BLOCK_COMMENT, BRACKET, CODE_BLOCK_COMMENT, CODE_BLOCK_COMMENT_LEAD_SYMBOL, COMPACT_CLAUSE_EXPRESSION, CONCAT_LINE, CONJUNCTION, CURVE, C_BLOCK_COMMENT, DATA_BRACKET, EOI, FUNCTION, HALF_DENT, HASH, HEAD_SPACES, IDENTIFIER, INDENT, INDENT_EXPRESSION, INLINE_COMMENT, INTERPOLATE_EXPRESSION, INTERPOLATE_STRING, KEYWORD, LINE_COMMENT, MODULE, MODULE_HEADER, NEWLINE, NON_INTERPOLATE_STRING, NUMBER, OPERATOR_EXPRESSION, PAREN, PREFIX, PUNCTUATION, REGEXP, RIGHT_DELIMITER, SPACE, SPACES, SPACES_INLINE_COMMENT, SPACE_CLAUSE_EXPRESSION, SPACE_COMMENT, STRING, SUFFIX, SYMBOL, TAIL_COMMENT, UNDENT, base, begin, binaryOperatorDict, charset, colors, compileExp, conjMap, conjunctionHasOwnProperty, constant, dict, digitCharSet, digitChars, entity, escapeNewLine, extend, firstIdentifierCharSet, firstIdentifierChars, firstSymbolCharset, getOperatorExpression, hasOwnProperty, identifierCharSet, identifierChars, isArray, isConj, isKeyword, keywordHasOwnProperty, keywordMap, letterCharSet, letterChars, letterDigitSet, list2dict, makeOperatorExpression, prefixOperatorDict, str, suffixOperatorDict, taijiIdentifierCharSet, wrapInfo1, wrapInfo2, _ref, _ref1, _ref2,
  __slice = [].slice;
...
NUMBER = constant.NUMBER, STRING = constant.STRING, IDENTIFIER = constant.IDENTIFIER, SYMBOL = constant.SYMBOL, REGEXP = constant.REGEXP, HEAD_SPACES = constant.HEAD_SPACES, CONCAT_LINE = constant.CONCAT_LINE, PUNCTUATION = constant.PUNCTUATION, FUNCTION = constant.FUNCTION, C_BLOCK_COMMENT = constant.C_BLOCK_COMMENT, PAREN = constant.PAREN, BRACKET = constant.BRACKET, DATA_BRACKET = constant.DATA_BRACKET, CURVE = constant.CURVE, INDENT_EXPRESSION = constant.INDENT_EXPRESSION, NEWLINE = constant.NEWLINE, SPACES = constant.SPACES, INLINE_COMMENT = constant.INLINE_COMMENT, SPACES_INLINE_COMMENT = constant.SPACES_INLINE_COMMENT, LINE_COMMENT = constant.LINE_COMMENT, BLOCK_COMMENT = constant.BLOCK_COMMENT, CODE_BLOCK_COMMENT = constant.CODE_BLOCK_COMMENT, CONCAT_LINE = constant.CONCAT_LINE, NON_INTERPOLATE_STRING = constant.NON_INTERPOLATE_STRING, INTERPOLATE_STRING = constant.INTERPOLATE_STRING, EOI = constant.EOI, INDENT = constant.INDENT, UNDENT = constant.UNDENT, HALF_DENT = constant.HALF_DENT, MODULE_HEADER = constant.MODULE_HEADER, MODULE = constant.MODULE, SPACE_COMMENT = constant.SPACE_COMMENT, TAIL_COMMENT = constant.TAIL_COMMENT, SPACE = constant.SPACE, HASH = constant.HASH, RIGHT_DELIMITER = constant.RIGHT_DELIMITER, KEYWORD = constant.KEYWORD, CONJUNCTION = constant.CONJUNCTION, CODE_BLOCK_COMMENT_LEAD_SYMBOL = constant.CODE_BLOCK_COMMENT_LEAD_SYMBOL, PREFIX = constant.PREFIX, SUFFIX = constant.SUFFIX, BINARY = constant.BINARY, OPERATOR_EXPRESSION = constant.OPERATOR_EXPRESSION, COMPACT_CLAUSE_EXPRESSION = constant.COMPACT_CLAUSE_EXPRESSION, SPACE_CLAUSE_EXPRESSION = constant.SPACE_CLAUSE_EXPRESSION, INTERPOLATE_EXPRESSION = constant.INTERPOLATE_EXPRESSION;
...
exports.Parser = function() {
  var atLineHead, atStatementHead, atomTokenTypes, binaryDictOperator, binaryOperatorMemoIndex, bracketVariantMap, breakContinueStatement, c, canFollowSuffix, canFollowSufix, caseClauseOfSwitchStatement, catchClause, char, concatenating, conjClause, cursor, curveVariantMap, customBinaryOperatorFnMap, decimal, elseClause, endCursorOfDynamicBlockStack, environment, eof, error, expect, expectChar, expectClauseEnd, expectIdentifier, expectIndentConj, expectOneOfWords, expectWord, expression, expressionMemoIndex, finallyClause, follow, followMatch, followSequence, indent, indentExpression, interpolateStringPiece, isCallable, isClauseEnd, isIdentifier, itemToParameter, key, keywordTestExpressionBodyStatement, keywordThenElseStatement, leadTokenClause, leadWordClauseMap, leftCBlockComment, leftIndentBlockComment, leftInterpolatedString, leftNonInterpolatedString, leftRawInterpolatedString, leftRawNonInterpolatedString, leftRegexp, letLikeStatement, lexError, lineStart, lineno, literal, matchToken, maybeOneOfWords, memoIndex, memoMap, newline, nextToken, nonInterpolatedStringLine, operatorExpression, parenVariantMap, parser, predefined, rawInterpolateStringPiece, rawNonInterpolatedStringLine, repeatedCharToken, seperatorList, shadowToken, skipInlineSpace, skipSpaceLines, spaceClauseExpression, spaceComma, symbol2clause, symbolStopChars, text, textLength, thenClause, throwReturnStatement, toParameters, token, tokenFnMap, tokenOnAtChar, tokenOnBackSlashChar, tokenOnColonChar, tokenOnDotChar, tokenOnDoubleQuoteChar, tokenOnForwardSlashChar, tokenOnIdentifierChar, tokenOnLeftBracketChar, tokenOnLeftParenChar, tokenOnNewlineChar, tokenOnNumberChar, tokenOnPunctuationChar, tokenOnRightDelimiterChar, tokenOnSingleQuoteChar, tokenOnSpaceChar, tokenOnSymbolChar, unchangeable, word, wrapResult, _i, _j, _len, _len1, _ref3, _ref4;
  parser = this;
  ...
  tokenFnMap[' '] = tokenFnMap['\t'] = tokenOnSpaceChar = function() {
    var c, col, column, concatIndent, cur, dent, line, tkn;
    cur = cursor;
    line = lineno;
    col = cursor - lineStart;
    dent = indent;
    ...
    if (newline()) {
      ...
      if (indent > dent) {
        ...
        token.type = UNDENT;
        token.clauseEnd = token.sentenceEnd = true;
      } else {
        token.type = NEWLINE;
        token.clauseEnd = token.sentenceEnd = true;
      }
  ...
  tokenFnMap['\\'] = tokenOnBackSlashChar = function() {
    ...
    if (char === '\n' || char === '\r') {
      ...
      txt = text.slice(token.cursor, token.stopCursor);
      if (txt[2] === "'") {
        tkn = token;
        char = text[++cursor];
        token = {
  ...
  tokenFnMap['/'] = tokenOnForwardSlashChar = function() {
    var col, cur, ind, line, prev, t, value;
    cur = cursor;
    col = cursor - lineStart;
    char = text[++cursor];
    line = lineno;
  ...
  tokenFnMap[','] = tokenOnPunctuationChar = function() {
    var cur;
    cur = cursor;
    char = text[++cursor];
    return {
  ...
  tokenFnMap['"'] = tokenOnDoubleQuoteChar = function() {
    ...
    if (char === '"') {
      ...
      return token.next = leftInterpolatedString();
    }
  };
  this.leftRawInterpolatedString = leftRawInterpolatedString = function() {
  ...
  leftInterpolatedString = function() {
    var cur, indentInfo, line, literalStart, pieces, x;
    cur = cursor - 1;
    line = lineno;
  ...
  tokenFnMap['{'] = function() {
    ...
    cursor++;
    line = lineno;
    col = cursor - lineStart(ind = indent);
    start = token;
    matchToken();
  ...
  parser.expressionEnd = function(mode) {
    var type, value;
    return (value = token.value) === ',' || value === ';' || value === ':' || (type = token.type) === NEWLINE || type === UNDENT || type === EOI || (mode === INTERPOLATE_EXPRESSION && (char === "'" || char === '"'));
  };
  binaryOperatorMemoIndex = memoIndex;
  memoIndex += 5;
  this.binaryOperator = function(mode, x) {
    var fn, m, op, start, type;
    if (m = token[binaryOperatorMemoIndex + mode]) {
      token = m.next;
      return m.result;
    }
    if ((type = token.type) === EOI || type === UNDENT || type === RIGHT_DELIMITER) {
    ...
    token = start;
    if ((fn = customBinaryOperatorFnMap[type]) && (op = fn(mode, x))) {
      token[binaryOperatorMemoIndex + mode] = {
        result: op,
        next: token
      };
  ...
  binaryDictOperator = function(mode, x) {
    var op, opToken, opValue, pri, priInc, txt2, type, type1;
    if ((type1 = token.type) === SPACE) {
      if (mode === INTERPOLATE_EXPRESSION) {
        return;
      }
      priInc = 300;
      nextToken();
  ...
  this.expression = expression = function(mode, priority, leftAssoc) {
    var indexMode, op, opPri, result, start, tkn, x, y;
  ...
  this.interpolateExpression = function() {
    return parser.expression(INTERPOLATE_EXPRESSION, 600, true);
  };
  ...
  this.unaryExpressionClause = function() {
  ...
  this.expressionClause = function() {
    var line1, start, x;
    start = cursor;
    line1 = lineno;
    if ((x = parser.spaceClauseExpression())) {
  ...
  this.definitionSymbol = function() {
    var tail, value;
    if (token.type !== SYMBOL) {
      return;
    }
  ...
  this.definition = function() {
    ...
    if (!(token = parser.definitionSymbol())) {
      token = start;
      return;
    }
  ...
  isClauseEnd = function() {
    var type;
    return (type = token.type) === NEWLINE || type === UNDENT || type === EOI || token.value === ',' || token.value === ';';
  };
  this.clauseItem = function() {
    var item;
    if ((item = parser.compactClauseExpression())) {
      if (token.type === PAREN && (item = parser.definition())) {
        return item;
      } else {
        return token;
      }
  ...
  this.sequenceClause = function() {
    var clause, item, meetComma, start;
    start = token;
    clause = [];
  ...
  this.clause = function() {
    var blk, clause, clauses, exp, fn, head, op, originalToken, start, type;
    start = token;
    ...
    if (type === KEYWORD) {
      ...
      if (token.value === '#' && (blk = parser.lineBlock())) {
    ...
    if ((exp = parser.spaceClauseExpression()) && isCallable(head)) {
  ...
  expectClauseEnd = function(message) {
    var type, value;
    if ((value = token.value) !== ',' && value !== ';' && (type = token.type) !== NEWLINE && type !== UNDENT && type !== EOI) {
      return error(message);
    }
  };
