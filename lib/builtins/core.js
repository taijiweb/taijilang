__slice = [].slice;
var Parser, TaijiModule, addPrelude, argumentsLength, begin, compileExp, compileMetaListCode, convert, convertAssign, convertAssignRight, convertDefinition, convertEllipsisList, convertExps, convertIdentifier, convertList, convertMacro, convertParametersWithEllipsis, convertParserAttribute, convertParserExpression, declareVar, entity, error, extend, fs, idConvert, isArray, keywordConvert, makeSlice, metaCode, metaIndex, metaList, parser, quasiquote, return_, splitSpace, tjModule, transformToCode, undefinedExp, _ref, _ref1;
...
_ref1 = require('../compiler'), convert = _ref1.convert, convertList = _ref1.convertList, convertEllipsisList = _ref1.convertEllipsisList, convertExps = _ref1.convertExps, compileExp = _ref1.compileExp, transformToCode = _ref1.transformToCode, metaList = _ref1.metaList, metaCode = _ref1.metaCode;
...
exports['#='] = function(exp, env) {
  ...
  if (typeof left === 'string') {
    if (left[0] === '"') {
      return error('left side of assign should not be string');
    } else {
      env.set(left, left);
      value = convert(right, env);
      env.set(left, value);
      return '';
    }
  } else {
    return error('should not assign macro or meta value to non variable');
  }
};
...
exports['include!'] = function(exp, env) {
  var code, filePath, newEnv, parseMethod, raw, taijiModule;
  ...
  return exp = convert(exp.body, newEnv);
};

exports['use!'] = function(exp, env) {
  var alias, asName, code, envValue, filePath, inMeta, item, metaNames, moduleFn, name, names, newEnv, parseMethod, raw, resultExp, taijiModule, tjExports, useExp, _i, _j, _len, _len1;
  filePath = entity(exp[0]);
  filePath = filePath.slice(1, filePath.length - 1);
  taijiModule = new TaijiModule(filePath, env.module);
  alias = exp[1];
  ...
  for (_j = 0, _len1 = metaNames.length; _j < _len1; _j++) {
    item = metaNames[_j];
    name = item[0], asName = item[1];
    asName = entity(asName);
    name = entity(name);
    asName = asName || name;
    if (hasOwnProperty.call(tjExports, name)) {
      ...
      error('can not look up ' + name + ' from variable environment');
    }
  }
  return resultExp;
};

exports['export!'] = function(exp, env) {
  var eName, inMeta, isUsed, item, meta, name, result, value, _i, _len;
  meta = env.meta;
  isUsed = env.get('$$taijiModule');
  result = [];
  for (_i = 0, _len = exp.length; _i < _len; _i++) {
    item = exp[_i];
    name = item[0], value = item[1], inMeta = item[2];
    eName = entity(name);
    if (inMeta) {
      if (value) {
        if (isUsed) {
          ...
          env.set(eName, eval(compileExp(value, meta.env)));
        }
      } else {
        if (isUsed) {
          env.module.exports[eName] = env.get(eName);
        }
      }
    } else {
      if (value) {
        if (isUsed) {
          result.push(['=', ['attribute!', '$$taijiModule', name], value]);
        } else {
          result.push(['=', ['attribute!', 'exports', name], value]);
        }
      } else {
        if (isUsed) {
          result.push(['=', ['attribute!', '$$taijiModule', name], name]);
        } else {
          result.push(['=', ['attribute!', 'exports', name], name]);
        }
      }
    }
  }
...
convertDefinition = function(exp, env, mode) {
  var body, defaultList, ellipsis, exp0, exp1, i, newEnv, param, params, result, scope, _i, _len, _this;
  ...
  if (ellipsis !== void 0) {
    ...
    params = (function() {
      ...
      _results = [];
      for (i = _j = 0, _len1 = exp0.length; _j < _len1; i = ++_j) {
        param = exp0[i];
        param = entity(param);
        _results.push(scope[param] = param);
      }
      return _results;
    })();
    body = defaultList;
  ...
  if (mode === '=>' || mode === '\\=>') {
    result = ['begin!', ['=', _this, 'this'], ['function', params, body]];
  } else {
...
exports['->'] = function(exp, env) {
  return convertDefinition(exp, env, '->');
};
...
exports['quasiquote!'] = quasiquote = function(exp, env) {
  var e, exp0, head, meetSplice, result, _i, _len, _ref2;
  exp0 = exp[0];
  if (!isArray(exp0)) {
...
compileMetaListCode = function(exp, env) {
  var meta;
  meta = env.meta;
...
exports['##'] = function(exp, env) {
  return compileMetaListCode(exp[0], env);
};
...
exports['#'] = function(exp, env) {
  ...
  return compileMetaListCode(exp0, env);
};
