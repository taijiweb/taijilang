  ...
  if (left.kind === SYMBOL) {
    ...
    if (env.hasLocal(leftValue)) {
      ...
      if (left.outer) {
        ...
        left["const"] = true;
        return ['begin!', ['var', left], ['=', left, convert(exp[2], env)], left];
      } else {
        return ['=', left, convert(exp[2], env)];
      }
    } else {
      env.set(leftValue, left = env.newVar(leftValue));
      left["const"] = true;
    ...
    return [exp[0], convert(left, env), convert(exp[2], env)];
  }
};
...
exports['directLineComment!'] = function(exp, env) {
  return ['directLineComment!', exp[1]];
};
...
exports['directCBlockComment!'] = function(exp, env) {
  return ['directCBlockComment!', exp[1]];
};
...
exports['label!'] = function(exp, env) {
  return ['label!', convertIdentifier(exp[1]), convert(exp[2], env)];
};
...
binaryConverters['concat()'] = function(exp, env, compiler) {
  return ['call!', convert(exp[2], env), convert(exp[3], env)];
};
