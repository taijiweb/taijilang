...
exports['newvar!'] = function(exp, env) {
  var x;
  return '"' + env.newVar((x = entity(exp[0])).slice(1, x.length - 1)) + '"';
};
      ...
      if (head === 'unquote-splice') {
        if (level === 0) {
          ...
          if (!meetSpice) {
            ...
            result = ['call!', ['attribute', result, 'concat'], [['list!', quasiquote(e[1], env, level - 1)]]];
          }
        }
      } else {
        if (!meetSplice) {
          ...
          if (head === 'unquote!') {
            ...
            result = ['call!', ['attribute', result, 'concat'], [['list!', ['quasiquote!', quasiquote(e[1], env, level + 1)]]]];
          } else {
            result = ['call!', ['attribute!', result, 'concat'], [['list!', [quasiquote(e, env, level)]]]];
          }
        }
      }
    } else {
