var LIST, Parser, SYMBOL, VALUE, addPrelude, argumentsLength, begin, binaryConverters, commaList, compileError, compileExp, constant, convert, convertArgumentList, convertAssign, convertDefinition, convertExps, convertIdentifier, convertList, convertMetaExp, convertParametersWithEllipsis, convertParserAttribute, convertParserExpression, declareVar, error, extend, fs, idConvert, isArray, keywordConvert, makeSlice, metaIndex, metaProcess, metaProcessConvert, nonMetaCompileExp, norm, prefixConverters, quasiquote, return_, splitSpace, str, suffixConverters, sym, trace, transformToCode, undefinedExp, _fn, _i, _len, _ref, _ref1, _ref2;

_ref = require('../utils'), norm = _ref.norm, constant = _ref.constant, str = _ref.str, trace = _ref.trace, convertIdentifier = _ref.convertIdentifier, begin = _ref.begin, return_ = _ref.return_, error = _ref.error, isArray = _ref.isArray, error = _ref.error, extend = _ref.extend, splitSpace = _ref.splitSpace, undefinedExp = _ref.undefinedExp, addPrelude = _ref.addPrelude;
...
convertAssign = function(exp, env) {
  var e, ellipsis, i, item, left, leftLength, leftValue, n, result, right, rightLength, v, x, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _len6, _m, _n, _o, _ref2, _ref3;
  left = exp[1];
  if (left.kind === SYMBOL) {
...
exports['letrec!'] = function(exp, env) {
  ...
  for (_i = 0, _len = _ref2.length; _i < _len; _i++) {
    x = _ref2[_i];
    scope[x0 = x[0].value] = var1 = newEnv.newVar(x0);
    result.push([norm('var'), var1]);
  }
  _ref3 = exp[1];
...
exports['if'] = function(exp, env) {
  if (exp[3] !== void 0) {
    return [norm('if'), convert(exp[1], env), convert(exp[2], env), convert(exp[3], env)];
  } else {
    return [norm('if'), convert(exp[1], env), convert(exp[2], env), undefinedExp];
  }
};
...
prefixConverters['@'] = function(exp, env) {
  return norm(['attribute!', 'this', exp[2]]);
};

prefixConverters['::'] = function(exp, env) {
  return norm(['attribute!', ['attribute!', 'this', 'prototype'], exp[2]]);
};
...
suffixConverters['::'] = function(exp, env) {
  return norm(['attribute!', convert(exp[2], env), 'prototype']);
};
...
binaryConverters['::'] = function(exp, env, compiler) {
  return norm(['attribute!', ['attribute!', convert(exp[2], env), 'prototype'], exp[3]]);
};
...
commaList = function(exp) {
  var exp2, result;
  exp2 = exp[2];
  if (exp2.kind === LIST && exp2[0].value === 'binary!' && exp2[1].value === ',') {
