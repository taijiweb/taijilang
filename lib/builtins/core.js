var Parser, addPrelude, argumentsLength, begin, binaryConverters, compileExp, convert, convertArgumentList, convertAssign, convertAssignRight, convertDefinition, convertExps, convertIdentifier, convertList, convertMetaExp, convertParametersWithEllipsis, convertParserAttribute, convertParserExpression, declareVar, entity, error, extend, fs, idConvert, isArray, keywordConvert, makeSlice, metaIndex, metaProcess, metaProcessConvert, nonMetaCompileExp, norm, quasiquote, return_, splitSpace, sym, transformToCode, undefinedExp, _fn, _i, _len, _ref, _ref1, _ref2;

norm = require('../utils').norm;

__slice = [].slice;

fs = require('fs');

_ref = require('../utils'), convertIdentifier = _ref.convertIdentifier, entity = _ref.entity, begin = _ref.begin, return_ = _ref.return_, error = _ref.error, isArray = _ref.isArray, error = _ref.error, extend = _ref.extend, splitSpace = _ref.splitSpace, undefinedExp = _ref.undefinedExp, addPrelude = _ref.addPrelude;

Parser = require('../parser').Parser;

_ref1 = require('../compiler'), convert = _ref1.convert, convertList = _ref1.convertList, convertArgumentList = _ref1.convertArgumentList, convertExps = _ref1.convertExps, compileExp = _ref1.compileExp, nonMetaCompileExp = _ref1.nonMetaCompileExp, transformToCode = _ref1.transformToCode, metaProcessConvert = _ref1.metaProcessConvert, metaProcess = _ref1.metaProcess;
...
exports['metaConvertVar!'] = function(exp, env) {
  return [norm('metaConvertVar!', exp[0])];
};
...
exports['newvar!'] = function(exp, env) {
  var x;
  return '"' + env.newVar((x = entity(exp[0])).slice(1, x.length - 1)).symbol + '"';
};
  ...
  if (typeof eLeft === 'string') {
    if (eLeft[0] === '"') {
      ...
      if (left.outer) {
        ...
        left["const"] = true;
        return norm(['begin!', ['var', left], ['=', left, convertAssignRight(right, env)], left]);
      } else {
        return norm(['=', left, convertAssignRight(right, env)]);
      }
    } else {
      env.set(eLeft, left = env.newVar(eLeft));
      left["const"] = true;
...
exports['hashAssign!'] = function(exp, env) {
  var exp0, exp1, result, vObj, x, _i, _len;
  ...
  if (exp0.length > 1) {
    ...
    return convert(norm(['=', exp0[0], ['attribute!', exp1, exp0[0]]]), env);
  }
};

exports['#='] = function(exp, env) {
  return ['##', convert([norm('='), exp[0], exp[1]], env)];
};
...
exports['letloop!'] = function(exp, env) {
  ...
  bindings = (function() {
    ...
    for (_j = 0, _len1 = exp0.length; _j < _len1; _j++) {
      ...
      _results.push([scope[entity(x[0])], return_(convert(value[2], fnBodyEnv))]);
    }
    return _results;
  })();
...
exports['if'] = function(exp, env) {
  if (exp[2] !== void 0) {
    return [norm('if'), convert(exp[0], env), convertList(exp[1], env), convertList(exp[2], env)];
  } else {
    return [norm('if'), convert(exp[0], env), convertList(exp[1], env)];
  }
};
...
exports['directLineComment!'] = function(exp, env) {
  return [norm('directLineComment!'), exp[0]];
};
...
exports['directCBlockComment!'] = function(exp, env) {
  return [norm('directCBlockComment!'), exp[0]];
};
...
exports['quote!'] = function(exp, env) {
  return [norm('quote!'), entity(exp[0])];
};
...
exports['label!'] = function(exp, env) {
  return [norm('label!'), convertIdentifier(exp[0]), convert(exp[1], env)];
};
...
quasiquote = function(exp, env, level) {
  var e, exp0, head, item, meetSplice, result, _j, _len1, _ref3;
  exp0 = exp[0];
  if (!isArray(exp0)) {
    return JSON.stringify(entity(exp0));
  } else if (!exp0.length) {
    return [];
  }
  if ((head = entity(exp0[0])) === 'unquote!' || head === 'unquote-splice') {
    return convert(exp0[1], env, level - 1);
  } else if (head === 'quasiquote!') {
    return norm(['list!', '"quasiquote!"', quasiquote(exp0[1], env, level + 1)]);
  }
  result = ['list!'];
  meetSplice = false;
  _ref3 = exp[0];
...
exports['eval!'] = function(exp, env) {
  ...
  if (isArray(exp0)) {
    if (exp0[0] === 'quote!') {
      ...
      return ['call!', 'eval', [nonMetaCompileExp(exp0, env)]];
    }
    ...
    if (exp0[0] === '"') {
      ...
      return norm(['call!', 'eval', exp0]);
    }
  } else {
    return norm(['call!', 'eval', [nonMetaCompileExp(exp0, env)]]);
  }
};

exports['metaEval!'] = function(exp, env) {
  return eval(nonMetaCompileExp(exp[0], env));
};
...
exports['%x'] = function(exp, env) {
  return convert(norm(['attribute!', '__$taiji_$_$parser__', exp[0]]), env);
};
...
exports['binary!'] = function(exp, env, compiler) {
  var op;
  return binaryConverters[(op = exp[0]).value](op, exp[1], exp[2], env);
};
