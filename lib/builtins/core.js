...
exports['hashAssign!'] = function(exp, env) {
  ...
  if (exp0.length > 1) {
    ...
    return convert(['=', exp0[0], ['attribute!', exp1, exp0[0]]], env);
  }
};
...
exports['if'] = function(exp, env) {
  if (exp[2] !== void 0) {
    return ['if', convert(exp[0], env), convertList(exp[1], env), convertList(exp[2], env)];
  } else {
    return ['if', convert(exp[0], env), convertList(exp[1], env)];
  }
};
...
exports['directLineComment!'] = function(exp, env) {
  return ['directLineComment!', exp[0]];
};
...
exports['directCBlockComment!'] = function(exp, env) {
  return ['directCBlockComment!', exp[0]];
};
...
quasiquote = function(exp, env, level) {
  ...
  if ((head = entity(exp0[0])) === 'unquote!' || head === 'unquote-splice') {
    ...
    return ['list!', '"quasiquote!"', quasiquote(exp0[1], env, level + 1)];
  }
  result = ['list!'];
  meetSplice = false;
  _ref3 = exp[0];
