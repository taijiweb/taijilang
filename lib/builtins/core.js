var LIST, Parser, SYMBOL, VALUE, addPrelude, argumentsLength, begin, binaryConverters, compileExp, constant, convert, convertArgumentList, convertAssign, convertDefinition, convertExps, convertIdentifier, convertList, convertMetaExp, convertParametersWithEllipsis, convertParserAttribute, convertParserExpression, declareVar, entity, error, extend, fs, idConvert, isArray, keywordConvert, makeSlice, metaIndex, metaProcess, metaProcessConvert, nonMetaCompileExp, norm, quasiquote, return_, splitSpace, str, sym, transformToCode, undefinedExp, _fn, _i, _len, _ref, _ref1, _ref2, _ref3;

_ref = require('../utils'), norm = _ref.norm, constant = _ref.constant, str = _ref.str;
...
convertAssign = function(exp, env) {
  var e, ellipsis, i, item, left, leftLength, leftValue, n, result, rightLength, v, x, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _len6, _m, _n, _o, _ref3, _ref4;
  left = exp[1];
  if (left.kind === SYMBOL) {
    ...
    if (env.hasLocal(leftValue)) {
      ...
      if (left.outer) {
        ...
        left["const"] = true;
        return norm(['begin!', ['var', left], ['=', left, convert(right, env)], left]);
      } else {
        return norm(['=', left, convert(right, env)]);
      }
    } else {
      env.set(leftValue, left = env.newVar(leftValue));
      left["const"] = true;
