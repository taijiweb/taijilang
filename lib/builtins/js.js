...
_ref = require('../utils'), isArray = _ref.isArray, error = _ref.error, entity = _ref.entity, begin = _ref.begin, undefinedExp = _ref.undefinedExp, pushExp = _ref.pushExp, identifierCharSet = _ref.identifierCharSet, norm = _ref.norm;
...
exports['index!'] = function(exp, env) {
  ...
  return norm(['index!', convert(exp[0], env), convert(exp[1], env)]);
};
...
idBinaryConvert = function(op, left, right, env) {
  return norm(['binary!', op, convert(left, env), convert(right, env)]);
};
...
for (_i = 0, _len = _ref2.length; _i < _len; _i++) {
  ...
  binaryConverters['='] = function(op, exp1, exp2, env) {
    return [norm('='), convert(exp1, env), convert(exp2, env)];
  };
}
...
_fn = function(symbol) {
  return exports[symbol] = function(exp, env) {
    if (exp.length === 2) {
      ...
      return norm(['prefix!', symbol]).concat(convert(exp[0], env));
    }
  };
};
...
for (_k = 0, _len2 = _ref4.length; _k < _len2; _k++) {
  ...
  binaryConverters[op] = function(op, exp1, exp2, env) {
    return [norm('binary!'), op, convert(exp1, env), convert(exp2, env)];
  };
}
...
prefix = function(symbol) {
  return function(exp, env) {
    return [norm('prefix!'), norm(symbol), convert(exp[0], env)];
  };
};
...
suffix = function(symbol) {
  return function(exp, env) {
    return [norm('suffix!'), norm(symbol), convert(exp[0], env)];
  };
};
...
exports['!!x'] = function(exp, env) {
  return norm(['prefix!', '!', ['prefix!', '!', convert(exp[0], env)]]);
};
...
exports['jsvar!'] = function(exp, env) {
  ...
  return norm(['jsvar!', exp[0]]);
};

exports['return'] = function(exp, env) {
  return norm(['return', convert(exp[0], env)]);
};
