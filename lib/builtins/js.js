var begin, binary, call, convert, convertForInOf, convertHash, convertInterpolatedString, convertList, entity, error, identifierCharSet, isArray, prefix, suffix, symbol, undefinedExp, _fn, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _m, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6;
...
exports['forIn!'] = function(exp, env) {
  ...
  if (item[0] !== 'metaConvertVar!' && !env.hasFnLocal(item.value)) {
    env.set(item.value, item);
    item = ['var', item];
  } else {
    item = convert(item, env);
  }
  return ['forIn!', item, convert(range, env), convert(body, env)];
};

exports['forIn!!'] = function(exp, env) {
  var body, index, item, range, result, t;
  item = exp[0], index = exp[1], range = exp[2], body = exp[3];
  if (!index) {
    return convert(['forIn!', item, range, body], env);
  } else {
    result = [];
