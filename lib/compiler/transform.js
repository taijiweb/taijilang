...
ShiftAheadStatementInfo = (function() {
  ...
  ShiftAheadStatementInfo.prototype.addEffectOf = function(exp) {
    var exp1Value, fn, left, right, stmt0;
    if (this.polluted) {
      return;
    }
    ...
    if (stmt0 === '=') {
      ...
      left = exp[2];
      right = exp[3];
    } else if ((stmt0 === 'prefix!' || stmt0 === 'suffix!') && ((exp1Value = exp[1].value) === '++' || exp1Value === '--')) {
      left = exp[2];
    }
    if (left) {
      if (left.kind === SYMBOL) {
        this.affectVars[left.value] = true;
      } else {
        this.polluted = true;
        return;
      }
