var ShiftAheadStatementInfo, addStatementEffectFnMap, addStatementEffectList, begin, entity, error, extend, hasOwnProperty, idFn, isArray, isExpression, isExpressionList, mergeList, notExp, pushExp, statementHeadMap, str, toExpression, transform, transformAndOrExpression, transformDirectExpression, transformExpression, transformExpressionFnMap, transformExpressionList, transformExpressionSequence, transformFnMap, transformInterpolatedString, transformReturnExpression, transformUnaryExpression, undefinedExp, useTransformExpression, wrapInfo1, _ref,
  __slice = [].slice;

_ref = require('../utils'), str = _ref.str, isArray = _ref.isArray, extend = _ref.extend, error = _ref.error, mergeList = _ref.mergeList, begin = _ref.begin, pushExp = _ref.pushExp, notExp = _ref.notExp, undefinedExp = _ref.undefinedExp, entity = _ref.entity, wrapInfo1 = _ref.wrapInfo1;
...
addStatementEffectFnMap = {
  ...
  'prefix!': function(info, exp) {
    return info.addEffectOf(exp[2]);
  },
  'suffix!': function(info, exp) {
    return info.addEffectOf(exp[2]);
  },
...
ShiftAheadStatementInfo = (function() {
  function ShiftAheadStatementInfo(affectVars, polluted) {
    this.affectVars = affectVars;
    this.polluted = polluted;
  }

  ShiftAheadStatementInfo.prototype.addEffectOf = function(exp) {
    var eLeft, fn, left, right, stmt0;
    if (this.polluted) {
      return;
    }
    if (!exp || !exp.push) {
    ...
    if (stmt0 === '=') {
      left = exp[1];
      right = exp[2];
    } else if (stmt0 === 'augmentAssign!') {
      left = exp[2];
      right = exp[3];
    } else if ((stmt0 === 'prefix!' || stmt0 === 'suffix!') && (exp[1] === '++' || exp[1] === '--')) {
      left = exp[2];
    }
    if (left) {
      eLeft = entity(left);
      if (typeof eLeft === 'string') {
        this.affectVars[eLeft] = true;
      } else {
        this.polluted = true;
        return;
      }
      if (right) {
        this.addEffectOf(right);
      }
      return;
    }
    if (fn = addStatementEffectFnMap[stmt0]) {
      return fn(this, exp);
    } else {
      return addStatementEffectList(this, exp);
    }
  };

  ShiftAheadStatementInfo.prototype.maybeAffect = function(name) {
    return this.polluted || hasOwnProperty.call(this.affectVars, name);
  };

  ShiftAheadStatementInfo.prototype.copy = function() {
    return new ShiftAheadStatementInfo(extend({}, this.affectVars), this.polluted);
  };

  ShiftAheadStatementInfo.prototype.merge = function() {
    var name, shiftStmtInfo, shiftStmtInfoList, _i, _len;
    ...
    for (_i = 0, _len = shiftStmtInfoList.length; _i < _len; _i++) {
      ...
      if (shiftStmtInfo.polluted) {
        this.polluted = shiftStmtInfo.polluted;
        return;
      }
  ...
  return ShiftAheadStatementInfo;
...
exports.toExpression = toExpression = function(exp) {
  ...
  exp0 = exp[0];
  if (exp0 === 'if') {
    return ['?:', toExpression(exp[1]), toExpression(exp[2]), toExpression(exp[3])];
  }
  if (exp0 === 'begin!') {
    return ['comma!'].concat((function() {
...
transformAndOrExpression = function(exp, env, shiftStmtInfo) {
  ...
  if (!rightStmt) {
    return [leftStmt, ['binary!', exp[1], leftValue, rightValue]];
  }
  leftVar = env.ssaVar('x');
  resultVar = env.ssaVar('t');
  ifStmt = ['if', testFn(leftVar), ['=', resultVar, leftVar], begin([rightStmt, ['=', resultVar, rightValue]])];
  ifStmt[isAndOr] = true;
  stmt = begin([leftStmt, ['var', leftVar], ['=', leftVar, leftValue], ['var', resultVar], ifStmt]);
...
transformReturnExpression = function(exp, env, shiftStmtInfo) {
  ...
  return [begin([expStmt, [exp[0], value]]), void 0];
};
...
transformExpressionFnMap = {
  ...
  'binary!': function(exp, env, shiftStmtInfo) {
    ...
    return [begin([leftStmt, rightStmt]), ['binary!', exp[1], left, right]];
  },
  '=': function(exp, env, shiftStmtInfo) {
    var affected, exp1, leftExp, leftStmt, rightExp, rightStmt, t, _ref1, _ref2, _ref3, _ref4;
    exp1 = entity(exp[1]);
    if (typeof exp1 === 'string' && exp1 && exp1[0] !== '"') {
  ...
  'while': function(exp, env, shiftStmtInfo) {
    var bodyStmt, bodyValue, e, lst, stmt, whileStmt, _ref1, _ref2;
    lst = env.newVar('list');
    _ref1 = transformExpression(exp[1], env, shiftStmtInfo), stmt = _ref1[0], e = _ref1[1];
    _ref2 = transformExpression(exp[2], env, shiftStmtInfo), bodyStmt = _ref2[0], bodyValue = _ref2[1];
    whileStmt = ['while', 1, begin([stmt, ['if', notExp(e), 'break'], bodyStmt, pushExp(lst, bodyValue)])];
    return [begin([['var', lst], whileStmt]), lst];
  },
  ...
  'forIn!': function(exp, env, shiftStmtInfo) {
    var bodyStmt, bodyValue, forInStmt, lst, rangeStmt, rangeValue, varName, varStmt, _ref1, _ref2, _ref3;
    lst = env.newVar('list');
    _ref1 = transformExpression(exp[1], env, shiftStmtInfo), varStmt = _ref1[0], varName = _ref1[1];
    ...
    forInStmt = ['forIn!', varName, rangeValue, begin([bodyStmt, pushExp(lst, bodyValue)])];
    return [begin([['var', lst], varStmt, rangeStmt, forInStmt]), lst];
  },
  'cFor!': function(exp, env, shiftStmtInfo) {
    ...
    cForStmt = ['cFor!', initValue, void 0, void 0, begin([testStmt, ['if', notExp(testValue), ['break']], bodyStmt, pushExp(lst, bodyValue), stepStmt])];
...
transformExpression = function(exp, env, shiftStmtInfo) {
  var fn, result, t, x;
  if (!exp || !(x = entity(exp))) {
    return [void 0, exp];
  }
  ...
  if (Object.prototype.toString.call(exp) === '[object Array]') {
    if (fn = transformExpressionFnMap[exp[0]]) {
      ...
      return transformExpressionList(exp, env, shiftStmtInfo);
    }
  } else {
    return [void 0, exp];
  }
...
useTransformExpression = function(exp, env) {
  return begin(transformExpression(exp, env, new ShiftAheadStatementInfo({})));
};
...
transformFnMap = {
  ...
  'if': function(exp, env) {
    ...
    _ref1 = transformExpression(exp[1], env, new ShiftAheadStatementInfo({})), stmtTest = _ref1[0], testExp = _ref1[1];
  ...
  'while': function(exp, env) {
    ...
    _ref1 = transformExpression(exp[1], env, new ShiftAheadStatementInfo({})), stmt = _ref1[0], testExp = _ref1[1];
  ...
  'doWhile!': function(exp, env) {
    ...
    _ref1 = transformExpression(exp[2], env, new ShiftAheadStatementInfo({})), stmt = _ref1[0], testExp = _ref1[1];
    return ['doWhile!', begin([transform(exp[1], env), stmt]), testExp];
  },
  'forIn!': function(exp, env) {
    var hashExp, stmt, _ref1;
    _ref1 = transformExpression(exp[2], env, new ShiftAheadStatementInfo({})), stmt = _ref1[0], hashExp = _ref1[1];
    if (stmt) {
      return ['begin!', stmt, ['forIn!', transform(exp[1], env), hashExp, transform(exp[3], env)]];
    } else {
      return ['forIn!', transform(exp[1], env), hashExp, transform(exp[3], env)];
    }
  },
  'cFor!': function(exp, env) {
    var body, cForStmt, initStmt, initValue, stepStmt, stepValue, testStmt, testValue, _ref1, _ref2, _ref3;
    _ref1 = transformExpression(exp[1], env, new ShiftAheadStatementInfo({})), initStmt = _ref1[0], initValue = _ref1[1];
    _ref2 = transformExpression(exp[2], env, new ShiftAheadStatementInfo({})), testStmt = _ref2[0], testValue = _ref2[1];
    _ref3 = transformExpression(exp[3], env, new ShiftAheadStatementInfo({})), stepStmt = _ref3[0], stepValue = _ref3[1];
    ...
    return begin([initStmt, cForStmt]);
  },
