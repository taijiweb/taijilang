...
_ref = require('../utils'), str = _ref.str, extend = _ref.extend, begin = _ref.begin, pushExp = _ref.pushExp, notExp = _ref.notExp, undefinedExp = _ref.undefinedExp, assert = _ref.assert, hasOwnProperty = _ref.hasOwnProperty, commentPlaceholder = _ref.commentPlaceholder, trace = _ref.trace;
...
_ref2 = '../constant', VALUE = _ref2.VALUE, SYMBOL = _ref2.SYMBOL, LIST = _ref2.LIST, COMMAND = _ref2.COMMAND;
...
transformExpressionFnMap = {
  ...
  'string!': function(exp, env, shiftStmtInfo) {
    var e, es, esValue, s, stmt, _i, _len, _ref3, _ref4;
    _ref3 = transformExpressionList(exp.slice(1), env, shiftStmtInfo), stmt = _ref3[0], es = _ref3[1];
    if ((esValue = es.value) && esValue[0].kind === VALUE) {
      s = esValue[0];
    } else {
      s = norm(['call!', ['attribute!', ['jsvar!', 'JSON'], 'stringify'], [esValue[0]]]);
    }
    _ref4 = esValue.slice(1);
  ...
  'binary!': function(exp, env, shiftStmtInfo) {
    ...
    return [begin([leftStmt, rightStmt]), [norm('binary!'), exp[1], leftExp, rightExp]];
  },
  ...
  'new': function(exp, env, shiftStmtInfo) {
    ...
    return [begin([norm(['var', t = env.ssaVar('t')]), stmt, norm(['=', t, norm(['new', e])])]), t];
  },
  ...
  'var': function(exp, env, shiftStmtInfo) {
    ...
    if (exp[1] instanceof Array) {
      ...
      return [norm(['var', variable]), variable];
    } else {
      return [norm(exp), undefinedExp];
    }
  },
  ...
  'hash!': function(exp, env, shiftStmtInfo) {
    var e, stmt, _ref3;
    _ref3 = transformExpressionList(exp[1], env, shiftStmtInfo), stmt = _ref3[0], e = _ref3[1];
    return [stmt, ['hash!', e]];
  },
  'hashitem!': function(exp, env, shiftStmtInfo) {
    var e, stmt, _ref3;
    _ref3 = transformExpression(exp[2], env, shiftStmtInfo), stmt = _ref3[0], e = _ref3[1];
    return [stmt, norm(['hashitem!', exp[1], e])];
  },
  ...
  'doWhile!': function(exp, env, shiftStmtInfo) {
    var bodyStmt, bodyValue, doWhileStmt, e, lst, stmt, _ref3, _ref4;
    lst = env.newVar('list');
    _ref3 = transformExpression(exp[1], env, shiftStmtInfo), bodyStmt = _ref3[0], bodyValue = _ref3[1];
    _ref4 = transformExpression(exp[1], env, shiftStmtInfo), stmt = _ref4[0], e = _ref4[1];
  ...
  'cFor!': function(exp, env, shiftStmtInfo) {
    var bodyStmt, bodyValue, cForStmt, initStmt, initValue, lst, stepStmt, stepValue, testStmt, testValue, _ref3, _ref4, _ref5, _ref6;
    lst = env.newVar('list');
    _ref3 = transformExpression(exp[1], env, shiftStmtInfo), initStmt = _ref3[0], initValue = _ref3[1];
    _ref4 = transformExpression(exp[2], env, shiftStmtInfo), testStmt = _ref4[0], testValue = _ref4[1];
    _ref5 = transformExpression(exp[3], env, shiftStmtInfo), stepStmt = _ref5[0], stepValue = _ref5[1];
    _ref6 = transformExpression(exp[4], env, shiftStmtInfo), bodyStmt = _ref6[0], bodyValue = _ref6[1];
  ...
  'switch': function(exp, env, shiftStmtInfo) {
    var action, caseValues, cases, defaultStmt, defaultValue, e, e0, exp2, exp3, i, info, j, resultCases, resultVar, stmt, stmt00, stmtExp, switchStmt, testStmt, testValue, values, x, x1, _ref3, _ref4, _ref5;
    exp2 = exp[2];
    cases = [];
    i = exp2.length;
    while (--i >= 0) {
      ...
      while (--j >= 0) {
        x = e0[j];
        caseValues.unshift((_ref3 = transformExpression(x, env, info = shiftStmtInfo.copy()), stmt = _ref3[0], x1 = _ref3[1], _ref3));
        if (info.polluted) {
          shiftStmtInfo.polluted = true;
        }
      }
    ...
    if (exp3 = exp[3]) {
      _ref4 = transformExpression(exp3, env, info = shiftStmtInfo.copy()), defaultStmt = _ref4[0], defaultValue = _ref4[1];
      if (info.polluted) {
        shiftStmtInfo.polluted = true;
      }
    }
    _ref5 = transformExpression(exp[1], env, info = shiftStmtInfo), testStmt = _ref5[0], testValue = _ref5[1];
    resultVar = env.ssaVar('t');
    ...
    resultCases = (function() {
      ...
      for (_i = 0, _len = cases.length; _i < _len; _i++) {
        _ref6 = cases[_i], caseValues = _ref6[0], action = _ref6[1];
        values = (function() {
...
exports.toExpression = toExpression = function(exp) {
  var e, exp0Value, expValue, i, _i, _len;
  switch (exp.kind) {
    case VALUE:
    case SYMBOL:
    case COMMAND:
    ...
    case LIST:
      ...
      for (i = _i = 0, _len = expValue.length; _i < _len; i = ++_i) {
        e = expValue[i];
        if (i === 0) {
          continue;
        }
...
exports.transformExpression = transformExpression = function(exp, env, shiftStmtInfo) {
  ...
  switch (exp.kind) {
    ...
    case LIST:
      ...
      return result;
    default:
      return compileError('wrong kind: ' + exp.kind + ' of ' + str(exp));
  }
};
...
transformFnMap = {
  ...
  'doWhile!': function(exp, env) {
    var stmt, testExp, _ref3;
    _ref3 = transformExpression(exp[2], env, new ShiftStatementInfo({}, {})), stmt = _ref3[0], testExp = _ref3[1];
  ...
  'cFor!': function(exp, env) {
    var body, cForStmt, initExp, initStmt, stepExp, stepStmt, testExp, testStmt, _ref3, _ref4, _ref5;
    _ref3 = transformExpression(exp[1], env, new ShiftStatementInfo({}, {})), initStmt = _ref3[0], initExp = _ref3[1];
    _ref4 = transformExpression(exp[2], env, new ShiftStatementInfo({}, {})), testStmt = _ref4[0], testExp = _ref4[1];
    _ref5 = transformExpression(exp[3], env, new ShiftStatementInfo({}, {})), stepStmt = _ref5[0], stepExp = _ref5[1];
...
exports.transform = transform = function(exp, env) {
  ...
  switch (exp.kind) {
    ...
    case LIST:
      ...
      expValue = exp.value;
      if ((fn = transformFnMap[expValue[0].value])) {
        result = fn(expValue, env);
      } else {
      ...
      return result;
    default:
      return 'transform: wrong kind: ' + exp.kind + ' of ' + str(exp);
  }
};
