...
transformForInExpression = function(exp, env, shiftStmtInfo) {
  var bodyStmt, bodyValue, forInStmt, lst, rangeStmt, rangeValue, _ref1, _ref2;
  lst = env.newVar('list');
  _ref1 = transformExpression(exp[3], env, shiftStmtInfo), bodyStmt = _ref1[0], bodyValue = _ref1[1];
...
transformExpressionFnMap = {
  ...
  'metaConvertVar!': function(exp, env, shiftStmtInfo) {
    ...
    if (shiftStmtInfo.maybeAffect(entity(exp[1]))) {
      return [begin([['var', t = env.ssaVar('t')], ['=', t, variable]]), t];
    } else {
