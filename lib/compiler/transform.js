...
transformExpressionFnMap = {
  ...
  'forIn!': function(exp, env, shiftStmtInfo) {
    ...
    forInStmt = ['forIn!', exp[1], rangeValue, begin([bodyStmt, pushExp(lst, bodyValue)])];
...
transformFnMap = {
  ...
  'forIn!': function(exp, env) {
    ...
    if (stmt) {
      return ['begin!', stmt, ['forIn!', exp[1], hashExp, transform(exp[3], env)]];
    } else {
      return ['forIn!', exp[1], hashExp, transform(exp[3], env)];
    }
  },
