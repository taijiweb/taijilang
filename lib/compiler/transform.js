...
assignEffectFn = function(info, exp) {
  var exp1Value, left;
  if (exp0Value === '=' || exp0Value === 'augmentAssign!') {
    left = exp[2];
    if (left.kind === SYMBOL) {

    } else {
      info.addEffectOf(right);
      return info.addEffectOf(left);
    }
  } else if ((exp0Value === 'prefix!' || exp0Value === 'suffix!') && ((exp1Value = exp[1].value) === '++' || exp1Value === '--')) {
    if (exp[1].kind === SYMBOL) {
      info.affectVars[exp1Value] = true;
    } else {
      this.polluted = true;
    }
  }
...
addStatementEffectFnMap = {
  ...
  'prefix!': function(info, exp) {
    var exp1Value;
    if ((exp1Value = exp[1].value) === '++' || exp1Value === '--') {
      if (exp[2].kind === SYMBOL) {
        this.affectVars[exp1Value] = true;
      } else {
...
ShiftAheadStatementInfo = (function() {
  ...
  ShiftAheadStatementInfo.prototype.addEffectOf = function(exp) {
    ...
    if (fn = addStatementEffectFnMap[exp0Value]) {
      return fn(this, exp);
    } else {
