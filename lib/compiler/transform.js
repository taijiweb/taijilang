var LIST, SYMBOL, ShiftStatementInfo, VALUE, assert, assignVarsOf, begin, commentPlaceholder, constant, entity, extend, hasOwnProperty, norm, notExp, pollutedOf, pushExp, str, transform, transformAndOrExpression, transformExpression, transformExpressionFnMap, transformExpressionList, transformExpressionSequence, transformFnMap, transformReturnThrowExpression, undefinedExp, varsOf, wrapInfo1, _ref, _ref1,
  __slice = [].slice;
...
_ref1 = require('./helper'), assignVarsOf = _ref1.assignVarsOf, varsOf = _ref1.varsOf, pollutedOf = _ref1.pollutedOf;

VALUE = constant.VALUE, SYMBOL = constant.SYMBOL, LIST = constant.LIST;

exports.ShiftStatementInfo = ShiftStatementInfo = (function() {
  function ShiftStatementInfo(assignVars, vars, polluted) {
    this.assignVars = assignVars;
    this.vars = vars;
    this.polluted = polluted;
  }

  ShiftStatementInfo.prototype.polluteOrAssign = function(name) {
...
transformExpressionFnMap = {
  ...
  'string!': function(exp, env, shiftStmtInfo) {
    ...
    for (_i = 0, _len = _ref3.length; _i < _len; _i++) {
      ...
      if (e.kind === VALUE) {
        ...
        str = norm(['binary!', '+', str, ['call!', ['attribute!', ['jsvar!', 'JSON'], 'stringify'], [e]]]);
      }
    }
    return [stmt, str];
  },
  ...
  '=': function(exp, env, shiftStmtInfo) {
    ...
    if (exp1.kind === SYMBOL) {
      ...
      return [begin([leftStmt, rightStmt, ['=', leftExp, rightExp]], ['var', (t = env.ssaVar('t'))], ['=', t, leftExp]), t];
    }
  },
  ...
  'if': function(exp, env, shiftStmtInfo) {
    var elseExp, ifStmt, resultVar, stmtElse, stmtTest, stmtThen, testExp, thenExp, _ref2, _ref3, _ref4;
    _ref2 = transformExpression(exp[1], env, shiftStmtInfo.copy()), stmtTest = _ref2[0], testExp = _ref2[1];
    _ref3 = transformExpression(exp[2], env, shiftStmtInfo.copy()), stmtThen = _ref3[0], thenExp = _ref3[1];
    _ref4 = transformExpression(exp[3], env, shiftStmtInfo.copy()), stmtElse = _ref4[0], elseExp = _ref4[1];
    resultVar = env.ssaVar('t');
  ...
  'try': function(exp, env, shiftStmtInfo) {
    var bodyStmt, bodyValue, catchAction, finallyAction, _ref2;
    _ref2 = transformExpression(exp[1], env, shiftStmtInfo.copy()), bodyStmt = _ref2[0], bodyValue = _ref2[1];
  ...
  'switch': function(exp, env, shiftStmtInfo) {
    var action, caseValues, cases, defaultStmt, defaultValue, e, e0, exp2, exp3, i, j, resultCases, resultVar, stmt, stmt00, stmtExp, switchStmt, testStmt, testValue, values, x, x1, _ref2, _ref3, _ref4;
    exp2 = exp[2];
    cases = [];
    i = exp2.length;
    while (--i >= 0) {
      ...
      while (--j >= 0) {
        x = e0[j];
        caseValues.unshift((_ref2 = transformExpression(x, env, shiftStmtInfo.copy()), stmt = _ref2[0], x1 = _ref2[1], _ref2));
      }
      cases.unshift([caseValues, transformExpression(e[1], env, shiftStmtInfo.copy())]);
    }
    if (exp3 = exp[3]) {
      _ref3 = transformExpression(exp3, env, shiftStmtInfo.copy()), defaultStmt = _ref3[0], defaultValue = _ref3[1];
    }
    _ref4 = transformExpression(exp[1], env, shiftStmtInfo.copy()), testStmt = _ref4[0], testValue = _ref4[1];
    resultVar = env.ssaVar('t');
