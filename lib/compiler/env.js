var Environment, SYMBOL, assert, constant, entity, extend, hasOwnProperty, identifierCharSet, javascriptKeywordSet, str, symbolLookupError, toIdentifier, _ref;

_ref = require('../utils'), extend = _ref.extend, str = _ref.str, javascriptKeywordSet = _ref.javascriptKeywordSet, entity = _ref.entity, identifierCharSet = _ref.identifierCharSet, assert = _ref.assert, constant = _ref.constant;

symbolLookupError = require('./helper').symbolLookupError;
...
exports.Environment = Environment = (function() {
  ...
  Environment.prototype.fnLocalNames = function(symbol) {
    var env, names;
    names = {};
    env = this;
    while (1) {
      if (env.scope && hasOwnProperty.call(env.scope, symbol)) {
  ...
  Environment.prototype.set = function(symbol, value) {
    ...
    assert(value.value && value.kind === SYMBOL, 'env.set: ' + str(value));
    functionInfo = this.getFunctionInfo();
  ...
  Environment.prototype.get = function(symbol) {
    ...
    while (env) {
      scope = env.scope;
      if (hasOwnProperty.call(scope, sym)) {
        return scope[sym];
      }
      env = env.parent;
    }
    return symbolLookupError(symbol);
  };
