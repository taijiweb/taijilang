var Environment, entity, extend, hasOwnProperty, identifierCharSet, javascriptKeywordSet, symbolLookupError, toIdentifier, _ref;

_ref = require('../utils'), extend = _ref.extend, javascriptKeywordSet = _ref.javascriptKeywordSet, entity = _ref.entity, identifierCharSet = _ref.identifierCharSet;
...
exports.symbolLookupError = symbolLookupError = function(exp, message) {
  if (message) {
    throw new Error(message + ': ' + JSON.stringify(exp));
  } else {
    throw new Error('symbol lookup error: ' + JSON.stringify(exp));
  }
};
...
exports.Environment = Environment = (function() {
  ...
  Environment.prototype.set = function(symbol, value) {
    ...
    eValue = entity(value);
    if (!functionInfo[eValue]) {
      functionInfo[eValue] = 1;
    }
