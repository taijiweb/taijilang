var binaryPriority, compileError, isArray, letterCharSet, makeCoder, string, symbolOf, trace, unaryPriority, valueOf, _ref;

_ref = require('../utils'), isArray = _ref.isArray, trace = _ref.trace, symbolOf = _ref.symbolOf, valueOf = _ref.valueOf, letterCharSet = _ref.letterCharSet;
...
exports.makeCoder = makeCoder = function(options) {
  var block, code, codeFnMap, indent, indentRow, indentWidth, list, nl, paren, prev, priParen, priParen2, priority, undent;
  ...
  return code = function(exp) {
    ...
    if (isArray(exp)) {
      return prev = codeFnMap[symbolOf(exp[0])](exp);
    } else {
      return prev = string(valueOf(exp));
    }
  };
};

exports.code = function(exp) {
