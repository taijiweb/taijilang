var $atMetaExpList, BLOCK_COMMENT, BRACKET, CODE_BLOCK_COMMENT, CONCAT_LINE, CURVE, DATA_BRACKET, Environment, FUNCTION, HALF_DENT, HEAD_SPACES, IDENTIFIER, INDENT, INDENT_EXPRESSION, INLINE_COMMENT, INTERPOLATE_STRING, LINE_COMMENT, MODULE, MODULE_HEADER, NEWLINE, NON_INTERPOLATE_STRING, NUMBER, PAREN, PUNCT, Parser, REGEXP, SPACES, SPACES_INLINE_COMMENT, STRING, SYMBOL, TaijiModule, UNDENT, analyze, begin, compileExp, compileExpNoOptimize, constant, convert, convertEllipsisList, convertExps, convertScopeExp, doAnalyze, entity, error, evaljs, exportsIndex, extend, formatTaijiJson, fs, getStackTrace, hasMeta, include, includeModuleFunctionCall, isArray, isMetaOperation, madeCall, madeConcat, metaCompile, metaConvert, metaConvertExport, metaConvertFnMap, metaConvertImport, metaInclude, metaProcess, metaTransform, nonMetaCompileExp, nonMetaCompileExpNoOptimize, optimize, optimizeExp, parseModule, parser, preprocessMetaConvertFnMap, pushExp, return_, str, taijiExports, tocode, transform, transformExp, transformToCode, undefinedExp, wrapInfo1, wrapMetaObjectFunctionCall, wrapModuleFunctionCall, _ref;

fs = require('fs');

_ref = require('../utils'), evaljs = _ref.evaljs, isArray = _ref.isArray, extend = _ref.extend, str = _ref.str, formatTaijiJson = _ref.formatTaijiJson, wrapInfo1 = _ref.wrapInfo1, entity = _ref.entity, pushExp = _ref.pushExp, undefinedExp = _ref.undefinedExp, begin = _ref.begin, return_ = _ref.return_;
...
convertScopeExp = function(exp, env) {
  var defExp, mode, newEnv, _i, _len, _ref1, _ref2;
  env.definitionExpList = [];
  exp = convert(exp, env);
  _ref1 = env.definitionExpList;
  for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
    _ref2 = _ref1[_i], defExp = _ref2[0], newEnv = _ref2[1], mode = _ref2[2];
    if (mode === 'letloop!') {
      defExp[1] = return_(convertScopeExp(begin(defExp[1]), newEnv));
    } else {
      if (mode[0] === '|') {
        defExp[2] = convertScopeExp(begin(defExp[2]), newEnv);
      } else {
        defExp[2] = return_(convertScopeExp(begin(defExp[2]), newEnv));
      }
    }
  }
  return exp;
...
exports.metaCompile = metaCompile = function(exp, metaExpList, env) {
  ...
  return code = nonMetaCompileExp(['->', ['__tjExp'], exp], env);
};
