var $atMetaExpList, BLOCK_COMMENT, BRACKET, CODE_BLOCK_COMMENT, CONCAT_LINE, CURVE, DATA_BRACKET, Environment, FUNCTION, HALF_DENT, HEAD_SPACES, IDENTIFIER, INDENT, INDENT_EXPRESSION, INLINE_COMMENT, INTERPOLATE_STRING, LINE_COMMENT, MODULE, MODULE_HEADER, NEWLINE, NON_INTERPOLATE_STRING, NUMBER, PAREN, PUNCT, Parser, REGEXP, SPACES, SPACES_INLINE_COMMENT, STRING, SYMBOL, TaijiModule, UNDENT, analyze, begin, compileExp, compileExpNoOptimize, constant, convert, convertEllipsisList, convertExps, entity, error, evaljs, exportsAttr, extend, formatTaijiJson, fs, getStackTrace, hasMeta, includeModuleFunctionCall, isArray, isMetaOperation, madeCall, madeConcat, metaCompile, metaConvert, metaConvertExport, metaConvertFnMap, metaConvertImport, metaConvertInclude, metaProcess, metaTransform, nonMetaCompileExp, nonMetaCompileExpNoOptimize, optimize, optimizeExp, parseModule, parser, preprocessMetaConvertFnMap, pushExp, str, taijiExports, tocode, transform, transformExp, transformToCode, undefinedExp, wrapInfo1, wrapMetaObjectFunctionCall, wrapModuleFunctionCall, _ref;
...
exportsAttr = function(attr) {
  return ['index!', taijiExports, attr];
};
...
metaConvertExport = function(exp, metaExpList, env) {
  var fn, inMeta, inObject, item, name, result, value, _i, _len;
  result = [];
  for (_i = 0, _len = exp.length; _i < _len; _i++) {
    item = exp[_i];
    name = item[0], value = item[1], inMeta = item[2], inObject = item[3];
    if (value === void 0) {
      value = name;
    }
    if (inMeta) {
...
includeModuleFunctionCall = function(exp, metaExpList, env) {
  var moduleVar, objLevelBody, objectLevelFunctionCall;
  moduleVar = ['metaConvertVar!', 'module'];
  objLevelBody = ['metaConvertVar!', 'objLevelBody'];
  objectLevelFunctionCall = metaConvert(['begin!', ['var', moduleVar], ['=', moduleVar, ['call!', ['->', [], ['begin!', ['=', 'exports', ['hash!']], exp, 'exports']], []]]], metaExpList, env);
  return ['begin!', ['var', objLevelBody], ['var', moduleVar], ['=', moduleVar, ['call!', ['->', [], ['begin!', ['=', 'exports', ['hash!']], ['=', ['@@', objLevelBody], objectLevelFunctionCall], 'exports']], []]], objLevelBody];
};

metaConvertInclude = function(exp, metaExpList, env) {
  var exportVar, fnCall, metaBegin, metaLevelAssignList, moduleFnCall, moduleVar, newEnv, objectLevelAssignList, _ref1;
  ...
  metaLevelAssignList = ['begin!', ['var', exportVar], ['forIn!', exportVar, moduleVar, ['if', ['call!', ['attribute!', '__hasProp', 'call'], [moduleVar, exportVar]], ['=', exportVar, ['index!', moduleVar, exportVar]]]]];
  objectLevelAssignList = metaConvert(metaLevelAssignList, metaExpList, env);
  return ['begin!', ['var', moduleFnCall], ['=', moduleFnCall, fnCall], metaLevelAssignList, ['list!', metaBegin, moduleFnCall, objectLevelAssignList]];
};

metaConvertImport = function(exp, metaExpList, env) {
  var asName, body, filePath, importItems, inMeta, item, metaModuleAlias, moduleCall, name, newEnv, objectModuleAlias, parseMethod, result, _i, _len, _ref1;
  filePath = exp[0], metaModuleAlias = exp[1], objectModuleAlias = exp[2], importItems = exp[3], parseMethod = exp[4];
  _ref1 = parseModule(entity(filePath), env, parseMethod), body = _ref1[0], newEnv = _ref1[1];
  result = [];
  moduleCall = wrapMetaObjectFunctionCall(body, metaExpList, env, metaModuleAlias, objectModuleAlias);
  result.push(moduleCall);
  for (_i = 0, _len = importItems.length; _i < _len; _i++) {
    item = importItems[_i];
    name = item[0], asName = item[1], inMeta = item[2];
    if (inMeta) {
      result.push(['=', asName, ['index!', metaModuleVar, name]]);
    } else {
      metaExpList.push(['=', asName, ['index!', metaModuleVar, name]]);
      result.push($atMetaExpList(env.metaIndex++));
    }
  }
  return begin(result);
};
...
metaConvertFnMap = {
  ...
  '#&': function(exp, metaExpList, env) {
    ...
    return begin([exp2, $atMetaExpList(env.metaIndex++)]);
  },
  '#=': function(exp, metaExpList, env) {
...
isMetaOperation = function(head) {
  return (head[0] === '#' && head[1] !== '-') || head === 'include!' || head === 'import!';
};
...
metaTransform = function(exp, metaExpList, env) {
  ...
  if (Object.prototype.toString.call(exp) === '[object Array]') {
    if (exp.length === 0) {
      return exp;
    } else if ((head = entity(exp[0])) && isMetaOperation(head)) {
...
exports.metaCompile = metaCompile = function(exp, metaExpList, env) {
  ...
  return code = nonMetaCompileExp(['return', exp], env);
};
