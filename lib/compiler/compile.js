...
exports.convert = convert = function(exp, env) {
  var e, exp0, expValue, head, result, tail;
  ...
  switch (exp.kind) {
    case LIST:
      ...
      switch (exp0.kind) {
        ...
        default:
          return compileError(exp0, 'convert: wrong kind: ' + exp0.kind);
      }
      break;
    case SYMBOL:
...
exports.convertArgumentList = convertArgumentList = function(exp, env) {
  var concated, concating, e, e1, ee1, ellipsis, exp0, exp01, i, item, piece, result, _i, _j, _len, _len1, _ref2;
  if (exp.length === 0) {
    return ['list!'];
  }
  if (exp.length === 1) {
  ...
  if (ellipsis === void 0) {
    return norm([norm('list!')].concat((function() {
...
metaInclude = function(exp, metaExpList, env) {
  var newEnv, _ref2;
  _ref2 = parseModule(entity(exp[1]), env, exp[2]), exp = _ref2[0], newEnv = _ref2[1];
...
metaConvertExport = function(exp, metaExpList, env) {
  var compileTime, item, name, result, runtime, value, _i, _len, _ref2;
  result = [];
  _ref2 = exp.slice(1);
...
metaConvertImport = function(exp, metaExpList, env) {
  var asName, cmd, compileTimeAssignList, filePath, fnCall, importItemList, metaBegin, metaImportItemList, metaModuleAlias, moduleFnCall, name, newEnv, parseMethod, runtimeAssignList, runtimeModuleAlias, _i, _j, _len, _len1, _ref2, _ref3, _ref4;
  ...
  _ref2 = parseModule(entity(filePath), env, parseMethod), exp = _ref2[0], newEnv = _ref2[1];
  if (!metaModuleAlias) {
  ...
  for (_i = 0, _len = metaImportItemList.length; _i < _len; _i++) {
    _ref3 = metaImportItemList[_i], name = _ref3[0], asName = _ref3[1];
  ...
  for (_j = 0, _len1 = importItemList.length; _j < _len1; _j++) {
    _ref4 = importItemList[_j], name = _ref4[0], asName = _ref4[1];
