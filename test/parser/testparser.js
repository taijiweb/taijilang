...
ndescribe("parse: ", function() {
  describe("clause: ", function() {
    ...
    return describe("do statement: ", function() {
      ...
      it('should parse do print 1; print 2; when a==1', function() {
        ...
        return expect(x).to.equal("[doWhile! [begin! [print 1] [print 2]] [binary! == a 1]]");
      });
      ...
      return it('should parse do print 1; print 2; until a==1', function() {
        ...
        return expect(x).to.equal("[doWhile! [begin! [print 1] [print 2]] [prefix ! [binary! == a 1]]]");
      });
    });
  });
  ...
  return describe("module: ", function() {
    ...
    describe("misc: ", function() {
      ...
      it('should parse \'##a=1; # ` ^ a', function() {
        return expect(parse('##a=1; # ` ^ a')).to.equal("[moduleBody! [begin! [prefix! ## [binary! = a 1]] [# [` [^ a]]]]]");
      });
      it('should parse \'a#=1; # ` ^ a', function() {
        return expect(parse('a#=1; # ` ^ a')).to.equal("[moduleBody! [begin! [binary! #= a 1] [# [` [^ a]]]]]");
      });
      it('should parse ^.a', function() {
        return expect(parse('^.a')).to.equal("[moduleBody! [prefix! ^ a]]");
      });
      return it('should parse \'##a=1; #.`.^a', function() {
        return expect(parse('##a=1; #.`.^a')).to.equal("[moduleBody! [begin! [prefix! ## [binary! = a 1]] [prefix! # [prefix! ` [prefix! ^ a]]]]]");
      });
    });
    describe("misc2: ", function() {
      ...
      it('should parse /. some comment', function() {
        return expect(parse('/. some comment')).to.equal("[moduleBody! undefined]");
      });
      ...
      return it('should parse `{ ^1, ^2 }', function() {
        return expect(parse('`{ ^1, ^2 }')).to.equal("[moduleBody! [prefix! ` [{} [begin! [prefix! ^ 1] [prefix! ^ 2]]]]]");
      });
    });
    describe("misc3: ", function() {
      ...
      it('should parse for x in [ 1 ] then print x', function() {
        return expect(parse('for x in [ 1 ] then print x')).to.equal("[moduleBody! [forIn! x undefined [[] [1]] [print x]]]");
      });
      it('should parse for x in [] then print x', function() {
        return expect(parse('for x in [] then print x')).to.equal("[moduleBody! [forIn! x undefined [[]] [print x]]]");
      });
      ...
      it('should parse m #= (a,b) -> `( ^a + ^b); m(1,2)', function() {
        return expect(parse('m #= (a,b) -> `( ^a + ^b ); m(1,2)')).to.equal("[moduleBody! [#= m [-> [() [binary! , a b]] [begin! [prefix! ` [() [binary! + [prefix! ^ a] [prefix! ^ b]]]] [binary! concat() m [() [binary! , 1 2]]]]]]]");
      });
    ...
    describe("new parser tests from samples: ", function() {
      ...
      return it('should parse and 1 /* inline comment */ 2', function() {
        var x;
    ...
    describe("new parser tests from samples 2: ", function() {
      ...
      it('should parse a = 2\nx = : 1', function() {
        return expect(function() {
          return parse('a = 2\nx = : 1');
        }).to["throw"](/expect the right side of assign/);
      });
      it('should parse a = 2\nx = 1', function() {
    ...
    describe("assign to outer scope var: ", function() {
      ...
      return it("should parse a=1; -> @@a=1", function() {
        return expect(parse('a=1; -> @@a=1')).to.equal("[moduleBody! [begin! [binary! = a 1] [-> [] [binary! = [prefix! @@ a] 1]]]]");
      });
    });
