...
describe "parse: ",  ->
  describe "clause: ",  ->
    ...
    describe "clauses which contain in curve or bracket: ",  ->
      it 'should parse [1]', ->
        expect(parse('[1] ')).to.equal "[list! 1]"
      it 'should parse {1}', ->
        expect(parse('{1} ')).to.equal "1"
      it 'should parse {1, 2}', ->
    ...
    describe 'meta: ', ->
      it 'should parse # if 1 then 1+2 else 3+4', ->
        expect(parse('# if 1 then 1+2 else 3+4')).to.equal "[# [if 1 [+ 1 2] [+ 3 4]]]"
      ...
      it 'should parse # #(1+2) + #(3+4)', ->
        expect(parse('# #(1+2) + #(3+4)')).to.equal "[# [+ [# [+ 1 2]] [# [+ 3 4]]]]"
  ...
  describe "line: ",  ->
    ...
    it '''should parse '#a=1;a''', ->
      expect(parse('#a=1;a')).to.equal "[[= [# a] 1] a]"
  ...
  describe "module: ",  ->
    ...
    describe "prefix: ",  ->
      it 'should parse +1', ->
        expect(parse('+1 ')).to.equal "[+x 1]"
      it 'should parse + 1', ->
        expect(parse('+ 1 ')).to.equal "[+x 1]"
      it 'should parse + + 1', ->
        expect(parse('+ + 1 ')).to.equal "[+x [+x 1]]"
      it 'should parse print + 1', ->
        expect(parse('print + 1 ')).to.equal "[+ print 1]"
      it 'should parse print: + 1', ->
        expect(parse('print: + 1 ')).to.equal "[print [+x 1]]"
    ...
    describe "line and indent operator: ",  ->
      ...
      it 'should parse ` { ^1 { ^2 ^&{3 4}}}', ->
        expect(parse('`{ ^1 { ^2 ^&{3 4}}}')).to.equal "[quasiquote! [[unquote! 1] [[unquote! 2] [unquote-splice [3 4]]]]]"
    ...
    describe "new parser tests from samples: ",  ->
      ...
      it "parse var a=1, b=2", ->
        expect(parse('var a=1, b=2')).to.equal "[var [a = 1] [b = 2]]"
    ...
    describe "assign to outer scope var: ",  ->
      ...
      it "should parse a=1; -> @@a=1", ->
        expect(parse('a=1; -> @@a=1')).to.equal "[begin! [= a 1] [-> [] [= [@@ a] 1]]]"

    it 'should parse if! 1 {break} {continue}', ->
      expect(parse("if! 1 {break} {continue}")).to.equal "[if! 1 [break] [continue]]"
