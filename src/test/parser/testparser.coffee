...
describe "parse: ",  ->
  describe "clause: ",  ->
    ...
    describe "function:",  ->
      ...
      it 'should parse -> and 1 2 ; and 3 4', ->
        ...
        expect(x).to.equal "[-> [] [[and 1 2] [and 3 4]]]"
      it 'should parse -> and 1 2 , and 3 4', ->
        x = parse('-> and 1 2 , and 3 4')
        expect(x).to.equal "[-> [] [[and 1 2] [and 3 4]]]"
      ...
      it 'should parse -> and 1 2 , and 3 4 -> print x ; print 5 6', ->
        ...
        expect(x).to.equal "[-> [] [[and 1 2] [and 3 4 [-> [] [[print x] [print 5 6]]]]]]"
    ...
    describe "do statement: ",  ->
      ...
      it 'should parse do print 1; print 2; when a==1', ->
        ...
        expect(x).to.equal "[doWhile! [[print 1] [print 2]] [binary! == a 1]]"
      ...
      it 'should parse do print 1; print 2; until a==1', ->
        ...
        expect(x).to.equal "[doWhile! [[print 1] [print 2]] [prefix ! [binary! == a 1]]]"
  ...
  describe "module: ",  ->
    ...
    describe "misc: ",  ->
      ...
      it '''should parse '##a=1; # ` ^ a''', ->
        expect(parse('##a=1; # ` ^ a')).to.equal "[moduleBody! [[prefix! ## [binary! = a 1]] [# [` [^ a]]]]]"
      it '''should parse 'a#=1; # ` ^ a''', ->
        expect(parse('a#=1; # ` ^ a')).to.equal "[moduleBody! [[binary! #= a 1] [# [` [^ a]]]]]"
      it '''should parse ^.a''', ->
        expect(parse('^.a')).to.equal "[moduleBody! [[prefix! ^ a]]]"
      it '''should parse '##a=1; #.`.^a''', ->
        expect(parse('##a=1; #.`.^a')).to.equal "[moduleBody! [[prefix! ## [binary! = a 1]] [prefix! # [prefix! ` [prefix! ^ a]]]]]"
      it 'should print \n 2 ', ->
      ...
      it 'should parse /. some comment', ->
        expect(parse('/. some comment')).to.equal "[moduleBody! []]"
      it 'should parse 1\n/. some comment', ->
      ...
      it 'should parse m #= (a,b) -> `( ^a + ^b); m(1,2)', ->
        expect(parse('m #= (a,b) -> `( ^a + ^b ); m(1,2)')).to.equal "[moduleBody! [[#= m [-> [() [binary! , a b]] [[prefix! ` [() [binary! + [prefix! ^ a] [prefix! ^ b]]]] [binary! concat() m [() [binary! , 1 2]]]]]]]]"
    ...
    describe "assign to outer scope var: ",  ->
      ...
      it "should parse a=1; -> @@a=1", ->
        expect(parse('a=1; -> @@a=1')).to.equal "[moduleBody! [[binary! = a 1] [-> [] [[binary! = [prefix! @@ a] 1]]]]]"
